...
// ApprovalRuleTemplateReconciler reconciles a ApprovalRuleTemplate object
type ApprovalRuleTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=codecommit.aws.kubeform.com,resources=approvalruletemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=codecommit.aws.kubeform.com,resources=approvalruletemplates/status,verbs=get;update;patch

func (r *ApprovalRuleTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("approvalruletemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApprovalRuleTemplate")
...
func (r *ApprovalRuleTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &codecommitv1alpha1.ApprovalRuleTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", codecommitv1alpha1.ApprovalRuleTemplate{}.APIVersion, codecommitv1alpha1.ApprovalRuleTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&codecommitv1alpha1.ApprovalRuleTemplate{}).
