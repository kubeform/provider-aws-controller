...
// ProxyEndpointReconciler reconciles a ProxyEndpoint object
...
// +kubebuilder:rbac:groups=db.aws.kubeform.com,resources=proxyendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=db.aws.kubeform.com,resources=proxyendpoints/status,verbs=get;update;patch

func (r *ProxyEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("proxyendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProxyEndpoint")
...
func (r *ProxyEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbv1alpha1.ProxyEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbv1alpha1.ProxyEndpoint{}.APIVersion, dbv1alpha1.ProxyEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbv1alpha1.ProxyEndpoint{}).
