...
// ProxyTargetReconciler reconciles a ProxyTarget object
type ProxyTargetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=db.aws.kubeform.com,resources=proxytargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=db.aws.kubeform.com,resources=proxytargets/status,verbs=get;update;patch

func (r *ProxyTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("proxytarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProxyTarget")
...
func (r *ProxyTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbv1alpha1.ProxyTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbv1alpha1.ProxyTarget{}.APIVersion, dbv1alpha1.ProxyTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbv1alpha1.ProxyTarget{}).
