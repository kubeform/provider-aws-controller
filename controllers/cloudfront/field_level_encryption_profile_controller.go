...
// FieldLevelEncryptionProfileReconciler reconciles a FieldLevelEncryptionProfile object
type FieldLevelEncryptionProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=fieldlevelencryptionprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=fieldlevelencryptionprofiles/status,verbs=get;update;patch

func (r *FieldLevelEncryptionProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fieldlevelencryptionprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FieldLevelEncryptionProfile")
...
func (r *FieldLevelEncryptionProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfrontv1alpha1.FieldLevelEncryptionProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfrontv1alpha1.FieldLevelEncryptionProfile{}.APIVersion, cloudfrontv1alpha1.FieldLevelEncryptionProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfrontv1alpha1.FieldLevelEncryptionProfile{}).
