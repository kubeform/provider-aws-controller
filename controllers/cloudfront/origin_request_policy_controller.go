...
// OriginRequestPolicyReconciler reconciles a OriginRequestPolicy object
type OriginRequestPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=originrequestpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=originrequestpolicies/status,verbs=get;update;patch

func (r *OriginRequestPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("originrequestpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OriginRequestPolicy")
...
func (r *OriginRequestPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfrontv1alpha1.OriginRequestPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfrontv1alpha1.OriginRequestPolicy{}.APIVersion, cloudfrontv1alpha1.OriginRequestPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfrontv1alpha1.OriginRequestPolicy{}).
