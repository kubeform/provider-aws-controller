...
// ResponseHeadersPolicyReconciler reconciles a ResponseHeadersPolicy object
type ResponseHeadersPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=responseheaderspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=responseheaderspolicies/status,verbs=get;update;patch

func (r *ResponseHeadersPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("responseheaderspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResponseHeadersPolicy")
...
func (r *ResponseHeadersPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfrontv1alpha1.ResponseHeadersPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfrontv1alpha1.ResponseHeadersPolicy{}.APIVersion, cloudfrontv1alpha1.ResponseHeadersPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfrontv1alpha1.ResponseHeadersPolicy{}).
