...
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=distributions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudfront.aws.kubeform.com,resources=distributions/status,verbs=get;update;patch

func (r *DistributionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("distribution", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Distribution")
...
func (r *DistributionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudfrontv1alpha1.Distribution{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudfrontv1alpha1.Distribution{}.APIVersion, cloudfrontv1alpha1.Distribution{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudfrontv1alpha1.Distribution{}).
