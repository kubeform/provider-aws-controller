...
// ConvertQueueReconciler reconciles a ConvertQueue object
type ConvertQueueReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=media.aws.kubeform.com,resources=convertqueues,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=media.aws.kubeform.com,resources=convertqueues/status,verbs=get;update;patch

func (r *ConvertQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("convertqueue", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConvertQueue")
...
func (r *ConvertQueueReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mediav1alpha1.ConvertQueue{}); err != nil {
			klog.Error(err, "unable to set up auditor", mediav1alpha1.ConvertQueue{}.APIVersion, mediav1alpha1.ConvertQueue{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mediav1alpha1.ConvertQueue{}).
