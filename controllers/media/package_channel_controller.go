...
// PackageChannelReconciler reconciles a PackageChannel object
type PackageChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=media.aws.kubeform.com,resources=packagechannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=media.aws.kubeform.com,resources=packagechannels/status,verbs=get;update;patch

func (r *PackageChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("packagechannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PackageChannel")
...
func (r *PackageChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mediav1alpha1.PackageChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", mediav1alpha1.PackageChannel{}.APIVersion, mediav1alpha1.PackageChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mediav1alpha1.PackageChannel{}).
...
func (r *PackageChannelReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PackageChannel")
			return nil
		}
		...
		resourceList := &mediav1alpha1.PackageChannelList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PackageChannel")
			return nil
		}
