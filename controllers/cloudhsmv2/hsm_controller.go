...
// HsmReconciler reconciles a Hsm object
type HsmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudhsmv2.aws.kubeform.com,resources=hsms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudhsmv2.aws.kubeform.com,resources=hsms/status,verbs=get;update;patch

func (r *HsmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hsm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Hsm")
...
func (r *HsmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudhsmv2v1alpha1.Hsm{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudhsmv2v1alpha1.Hsm{}.APIVersion, cloudhsmv2v1alpha1.Hsm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudhsmv2v1alpha1.Hsm{}).
