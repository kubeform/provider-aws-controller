...
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=authorizers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=authorizers/status,verbs=get;update;patch

func (r *AuthorizerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Authorizer")
...
func (r *AuthorizerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iotv1alpha1.Authorizer{}); err != nil {
			klog.Error(err, "unable to set up auditor", iotv1alpha1.Authorizer{}.APIVersion, iotv1alpha1.Authorizer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iotv1alpha1.Authorizer{}).
...
func (r *AuthorizerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for Authorizer")
			return nil
		}
		...
		resourceList := &iotv1alpha1.AuthorizerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Authorizer")
			return nil
		}
