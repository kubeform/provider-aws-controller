...
// RoleAliasReconciler reconciles a RoleAlias object
type RoleAliasReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=rolealias,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=rolealias/status,verbs=get;update;patch

func (r *RoleAliasReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rolealias", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RoleAlias")
...
func (r *RoleAliasReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iotv1alpha1.RoleAlias{}); err != nil {
			klog.Error(err, "unable to set up auditor", iotv1alpha1.RoleAlias{}.APIVersion, iotv1alpha1.RoleAlias{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iotv1alpha1.RoleAlias{}).
