...
// ThingPrincipalAttachmentReconciler reconciles a ThingPrincipalAttachment object
type ThingPrincipalAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=thingprincipalattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=thingprincipalattachments/status,verbs=get;update;patch

func (r *ThingPrincipalAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("thingprincipalattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ThingPrincipalAttachment")
...
func (r *ThingPrincipalAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iotv1alpha1.ThingPrincipalAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", iotv1alpha1.ThingPrincipalAttachment{}.APIVersion, iotv1alpha1.ThingPrincipalAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iotv1alpha1.ThingPrincipalAttachment{}).
