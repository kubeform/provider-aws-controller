...
// ThingGroupReconciler reconciles a ThingGroup object
type ThingGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=thinggroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iot.aws.kubeform.com,resources=thinggroups/status,verbs=get;update;patch

func (r *ThingGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("thinggroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ThingGroup")
...
func (r *ThingGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iotv1alpha1.ThingGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", iotv1alpha1.ThingGroup{}.APIVersion, iotv1alpha1.ThingGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iotv1alpha1.ThingGroup{}).
