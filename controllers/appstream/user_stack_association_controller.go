...
// UserStackAssociationReconciler reconciles a UserStackAssociation object
type UserStackAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=userstackassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=userstackassociations/status,verbs=get;update;patch

func (r *UserStackAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userstackassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserStackAssociation")
...
func (r *UserStackAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appstreamv1alpha1.UserStackAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", appstreamv1alpha1.UserStackAssociation{}.APIVersion, appstreamv1alpha1.UserStackAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appstreamv1alpha1.UserStackAssociation{}).
