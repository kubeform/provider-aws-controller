...
// DirectoryConfigReconciler reconciles a DirectoryConfig object
type DirectoryConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=directoryconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=directoryconfigs/status,verbs=get;update;patch

func (r *DirectoryConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("directoryconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DirectoryConfig")
...
func (r *DirectoryConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appstreamv1alpha1.DirectoryConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", appstreamv1alpha1.DirectoryConfig{}.APIVersion, appstreamv1alpha1.DirectoryConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appstreamv1alpha1.DirectoryConfig{}).
...
func (r *DirectoryConfigReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for DirectoryConfig")
			return nil
		}
		...
		resourceList := &appstreamv1alpha1.DirectoryConfigList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DirectoryConfig")
			return nil
		}
