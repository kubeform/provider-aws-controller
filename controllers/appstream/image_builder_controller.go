...
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=imagebuilders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=imagebuilders/status,verbs=get;update;patch

func (r *ImageBuilderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("imagebuilder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ImageBuilder")
...
func (r *ImageBuilderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appstreamv1alpha1.ImageBuilder{}); err != nil {
			klog.Error(err, "unable to set up auditor", appstreamv1alpha1.ImageBuilder{}.APIVersion, appstreamv1alpha1.ImageBuilder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appstreamv1alpha1.ImageBuilder{}).
