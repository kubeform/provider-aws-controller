...
// FleetStackAssociationReconciler reconciles a FleetStackAssociation object
type FleetStackAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=fleetstackassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appstream.aws.kubeform.com,resources=fleetstackassociations/status,verbs=get;update;patch

func (r *FleetStackAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fleetstackassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FleetStackAssociation")
...
func (r *FleetStackAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appstreamv1alpha1.FleetStackAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", appstreamv1alpha1.FleetStackAssociation{}.APIVersion, appstreamv1alpha1.FleetStackAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appstreamv1alpha1.FleetStackAssociation{}).
