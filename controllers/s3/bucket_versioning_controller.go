...
// BucketVersioningReconciler reconciles a BucketVersioning object
type BucketVersioningReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=bucketversionings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=bucketversionings/status,verbs=get;update;patch

func (r *BucketVersioningReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bucketversioning", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BucketVersioning")
...
func (r *BucketVersioningReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3v1alpha1.BucketVersioning{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3v1alpha1.BucketVersioning{}.APIVersion, s3v1alpha1.BucketVersioning{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3v1alpha1.BucketVersioning{}).
