...
// ObjectCopyReconciler reconciles a ObjectCopy object
type ObjectCopyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=objectcopies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=objectcopies/status,verbs=get;update;patch

func (r *ObjectCopyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("objectcopy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ObjectCopy")
...
func (r *ObjectCopyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3v1alpha1.ObjectCopy{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3v1alpha1.ObjectCopy{}.APIVersion, s3v1alpha1.ObjectCopy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3v1alpha1.ObjectCopy{}).
...
func (r *ObjectCopyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ObjectCopy")
			return nil
		}
		...
		resourceList := &s3v1alpha1.ObjectCopyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ObjectCopy")
			return nil
		}
