...
// BucketCorsConfigurationReconciler reconciles a BucketCorsConfiguration object
type BucketCorsConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=bucketcorsconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3.aws.kubeform.com,resources=bucketcorsconfigurations/status,verbs=get;update;patch

func (r *BucketCorsConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bucketcorsconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BucketCorsConfiguration")
...
func (r *BucketCorsConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3v1alpha1.BucketCorsConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3v1alpha1.BucketCorsConfiguration{}.APIVersion, s3v1alpha1.BucketCorsConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3v1alpha1.BucketCorsConfiguration{}).
