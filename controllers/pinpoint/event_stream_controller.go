...
// EventStreamReconciler reconciles a EventStream object
...
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=eventstreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=eventstreams/status,verbs=get;update;patch

func (r *EventStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eventstream", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventStream")
...
func (r *EventStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pinpointv1alpha1.EventStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", pinpointv1alpha1.EventStream{}.APIVersion, pinpointv1alpha1.EventStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pinpointv1alpha1.EventStream{}).
