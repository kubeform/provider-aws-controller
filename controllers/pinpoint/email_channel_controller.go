...
// EmailChannelReconciler reconciles a EmailChannel object
type EmailChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=emailchannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=emailchannels/status,verbs=get;update;patch

func (r *EmailChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("emailchannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EmailChannel")
...
func (r *EmailChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pinpointv1alpha1.EmailChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", pinpointv1alpha1.EmailChannel{}.APIVersion, pinpointv1alpha1.EmailChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pinpointv1alpha1.EmailChannel{}).
