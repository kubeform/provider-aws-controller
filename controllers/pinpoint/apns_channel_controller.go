...
// ApnsChannelReconciler reconciles a ApnsChannel object
type ApnsChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=apnschannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=apnschannels/status,verbs=get;update;patch

func (r *ApnsChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apnschannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApnsChannel")
...
func (r *ApnsChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pinpointv1alpha1.ApnsChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", pinpointv1alpha1.ApnsChannel{}.APIVersion, pinpointv1alpha1.ApnsChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pinpointv1alpha1.ApnsChannel{}).
...
func (r *ApnsChannelReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ApnsChannel")
			return nil
		}
		...
		resourceList := &pinpointv1alpha1.ApnsChannelList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ApnsChannel")
			return nil
		}
