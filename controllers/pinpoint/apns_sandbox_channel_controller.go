...
// ApnsSandboxChannelReconciler reconciles a ApnsSandboxChannel object
type ApnsSandboxChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=apnssandboxchannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=pinpoint.aws.kubeform.com,resources=apnssandboxchannels/status,verbs=get;update;patch

func (r *ApnsSandboxChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apnssandboxchannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApnsSandboxChannel")
...
func (r *ApnsSandboxChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &pinpointv1alpha1.ApnsSandboxChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", pinpointv1alpha1.ApnsSandboxChannel{}.APIVersion, pinpointv1alpha1.ApnsSandboxChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&pinpointv1alpha1.ApnsSandboxChannel{}).
...
func (r *ApnsSandboxChannelReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ApnsSandboxChannel")
			return nil
		}
		...
		resourceList := &pinpointv1alpha1.ApnsSandboxChannelList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ApnsSandboxChannel")
			return nil
		}
