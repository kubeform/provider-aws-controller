...
// FirehoseDeliveryStreamReconciler reconciles a FirehoseDeliveryStream object
...
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=firehosedeliverystreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=firehosedeliverystreams/status,verbs=get;update;patch

func (r *FirehoseDeliveryStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("firehosedeliverystream", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FirehoseDeliveryStream")
...
func (r *FirehoseDeliveryStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kinesisv1alpha1.FirehoseDeliveryStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", kinesisv1alpha1.FirehoseDeliveryStream{}.APIVersion, kinesisv1alpha1.FirehoseDeliveryStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kinesisv1alpha1.FirehoseDeliveryStream{}).
...
func (r *FirehoseDeliveryStreamReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for FirehoseDeliveryStream")
			return nil
		}
		...
		resourceList := &kinesisv1alpha1.FirehoseDeliveryStreamList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list FirehoseDeliveryStream")
			return nil
		}
