...
// StreamConsumerReconciler reconciles a StreamConsumer object
type StreamConsumerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=streamconsumers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=streamconsumers/status,verbs=get;update;patch

func (r *StreamConsumerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("streamconsumer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StreamConsumer")
...
func (r *StreamConsumerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kinesisv1alpha1.StreamConsumer{}); err != nil {
			klog.Error(err, "unable to set up auditor", kinesisv1alpha1.StreamConsumer{}.APIVersion, kinesisv1alpha1.StreamConsumer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kinesisv1alpha1.StreamConsumer{}).
