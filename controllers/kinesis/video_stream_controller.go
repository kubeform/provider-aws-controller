...
// VideoStreamReconciler reconciles a VideoStream object
type VideoStreamReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=videostreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=videostreams/status,verbs=get;update;patch

func (r *VideoStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("videostream", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VideoStream")
...
func (r *VideoStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kinesisv1alpha1.VideoStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", kinesisv1alpha1.VideoStream{}.APIVersion, kinesisv1alpha1.VideoStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kinesisv1alpha1.VideoStream{}).
