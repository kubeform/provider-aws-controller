...
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=streams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesis.aws.kubeform.com,resources=streams/status,verbs=get;update;patch
...
func (r *StreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Stream")
...
func (r *StreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kinesisv1alpha1.Stream{}); err != nil {
			klog.Error(err, "unable to set up auditor", kinesisv1alpha1.Stream{}.APIVersion, kinesisv1alpha1.Stream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kinesisv1alpha1.Stream{}).
