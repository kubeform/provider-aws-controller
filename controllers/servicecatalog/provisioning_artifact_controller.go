...
// ProvisioningArtifactReconciler reconciles a ProvisioningArtifact object
type ProvisioningArtifactReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=provisioningartifacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=provisioningartifacts/status,verbs=get;update;patch

func (r *ProvisioningArtifactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provisioningartifact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProvisioningArtifact")
...
func (r *ProvisioningArtifactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.ProvisioningArtifact{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.ProvisioningArtifact{}.APIVersion, servicecatalogv1alpha1.ProvisioningArtifact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.ProvisioningArtifact{}).
