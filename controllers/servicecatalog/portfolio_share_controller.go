...
// PortfolioShareReconciler reconciles a PortfolioShare object
type PortfolioShareReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=portfolioshares,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=portfolioshares/status,verbs=get;update;patch

func (r *PortfolioShareReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("portfolioshare", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PortfolioShare")
...
func (r *PortfolioShareReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.PortfolioShare{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.PortfolioShare{}.APIVersion, servicecatalogv1alpha1.PortfolioShare{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.PortfolioShare{}).
