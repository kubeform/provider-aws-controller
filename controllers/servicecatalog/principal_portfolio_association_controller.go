...
// PrincipalPortfolioAssociationReconciler reconciles a PrincipalPortfolioAssociation object
type PrincipalPortfolioAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=principalportfolioassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=principalportfolioassociations/status,verbs=get;update;patch

func (r *PrincipalPortfolioAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("principalportfolioassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrincipalPortfolioAssociation")
...
func (r *PrincipalPortfolioAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.PrincipalPortfolioAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.PrincipalPortfolioAssociation{}.APIVersion, servicecatalogv1alpha1.PrincipalPortfolioAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.PrincipalPortfolioAssociation{}).
