...
// OrganizationsAccessReconciler reconciles a OrganizationsAccess object
type OrganizationsAccessReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=organizationsaccesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=organizationsaccesses/status,verbs=get;update;patch

func (r *OrganizationsAccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationsaccess", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationsAccess")
...
func (r *OrganizationsAccessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.OrganizationsAccess{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.OrganizationsAccess{}.APIVersion, servicecatalogv1alpha1.OrganizationsAccess{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.OrganizationsAccess{}).
