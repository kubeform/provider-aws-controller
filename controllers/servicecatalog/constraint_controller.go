...
// ConstraintReconciler reconciles a Constraint object
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=constraints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=constraints/status,verbs=get;update;patch

func (r *ConstraintReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("constraint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Constraint")
...
func (r *ConstraintReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.Constraint{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.Constraint{}.APIVersion, servicecatalogv1alpha1.Constraint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.Constraint{}).
