...
// ProductPortfolioAssociationReconciler reconciles a ProductPortfolioAssociation object
type ProductPortfolioAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=productportfolioassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicecatalog.aws.kubeform.com,resources=productportfolioassociations/status,verbs=get;update;patch

func (r *ProductPortfolioAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("productportfolioassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProductPortfolioAssociation")
...
func (r *ProductPortfolioAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicecatalogv1alpha1.ProductPortfolioAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicecatalogv1alpha1.ProductPortfolioAssociation{}.APIVersion, servicecatalogv1alpha1.ProductPortfolioAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicecatalogv1alpha1.ProductPortfolioAssociation{}).
