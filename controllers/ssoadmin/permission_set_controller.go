...
// +kubebuilder:rbac:groups=ssoadmin.aws.kubeform.com,resources=permissionsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssoadmin.aws.kubeform.com,resources=permissionsets/status,verbs=get;update;patch

func (r *PermissionSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("permissionset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PermissionSet")
...
func (r *PermissionSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ssoadminv1alpha1.PermissionSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", ssoadminv1alpha1.PermissionSet{}.APIVersion, ssoadminv1alpha1.PermissionSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ssoadminv1alpha1.PermissionSet{}).
