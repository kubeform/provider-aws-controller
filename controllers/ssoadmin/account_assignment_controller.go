...
// AccountAssignmentReconciler reconciles a AccountAssignment object
type AccountAssignmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ssoadmin.aws.kubeform.com,resources=accountassignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssoadmin.aws.kubeform.com,resources=accountassignments/status,verbs=get;update;patch

func (r *AccountAssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountassignment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountAssignment")
...
func (r *AccountAssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ssoadminv1alpha1.AccountAssignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", ssoadminv1alpha1.AccountAssignment{}.APIVersion, ssoadminv1alpha1.AccountAssignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ssoadminv1alpha1.AccountAssignment{}).
