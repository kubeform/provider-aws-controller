...
// MysqlLayerReconciler reconciles a MysqlLayer object
type MysqlLayerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=mysqllayers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=mysqllayers/status,verbs=get;update;patch

func (r *MysqlLayerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mysqllayer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MysqlLayer")
...
func (r *MysqlLayerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.MysqlLayer{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.MysqlLayer{}.APIVersion, opsworksv1alpha1.MysqlLayer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.MysqlLayer{}).
