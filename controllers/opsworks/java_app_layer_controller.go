...
// JavaAppLayerReconciler reconciles a JavaAppLayer object
type JavaAppLayerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=javaapplayers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=javaapplayers/status,verbs=get;update;patch

func (r *JavaAppLayerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("javaapplayer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JavaAppLayer")
...
func (r *JavaAppLayerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.JavaAppLayer{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.JavaAppLayer{}.APIVersion, opsworksv1alpha1.JavaAppLayer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.JavaAppLayer{}).
