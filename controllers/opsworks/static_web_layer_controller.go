...
// StaticWebLayerReconciler reconciles a StaticWebLayer object
type StaticWebLayerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=staticweblayers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=staticweblayers/status,verbs=get;update;patch

func (r *StaticWebLayerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("staticweblayer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StaticWebLayer")
...
func (r *StaticWebLayerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.StaticWebLayer{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.StaticWebLayer{}.APIVersion, opsworksv1alpha1.StaticWebLayer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.StaticWebLayer{}).
