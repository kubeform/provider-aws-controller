...
// PhpAppLayerReconciler reconciles a PhpAppLayer object
type PhpAppLayerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=phpapplayers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=phpapplayers/status,verbs=get;update;patch

func (r *PhpAppLayerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("phpapplayer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PhpAppLayer")
...
func (r *PhpAppLayerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.PhpAppLayer{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.PhpAppLayer{}.APIVersion, opsworksv1alpha1.PhpAppLayer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.PhpAppLayer{}).
