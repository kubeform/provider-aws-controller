...
// EcsClusterLayerReconciler reconciles a EcsClusterLayer object
type EcsClusterLayerReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=ecsclusterlayers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=ecsclusterlayers/status,verbs=get;update;patch

func (r *EcsClusterLayerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ecsclusterlayer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EcsClusterLayer")
...
func (r *EcsClusterLayerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.EcsClusterLayer{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.EcsClusterLayer{}.APIVersion, opsworksv1alpha1.EcsClusterLayer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.EcsClusterLayer{}).
