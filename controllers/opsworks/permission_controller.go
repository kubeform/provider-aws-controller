...
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=permissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=opsworks.aws.kubeform.com,resources=permissions/status,verbs=get;update;patch
...
func (r *PermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Permission")
...
func (r *PermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &opsworksv1alpha1.Permission{}); err != nil {
			klog.Error(err, "unable to set up auditor", opsworksv1alpha1.Permission{}.APIVersion, opsworksv1alpha1.Permission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&opsworksv1alpha1.Permission{}).
