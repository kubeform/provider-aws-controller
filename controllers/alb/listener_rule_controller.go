...
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=listenerrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=listenerrules/status,verbs=get;update;patch

func (r *ListenerRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("listenerrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ListenerRule")
...
func (r *ListenerRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &albv1alpha1.ListenerRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", albv1alpha1.ListenerRule{}.APIVersion, albv1alpha1.ListenerRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&albv1alpha1.ListenerRule{}).
...
func (r *ListenerRuleReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ListenerRule")
			return nil
		}
		...
		resourceList := &albv1alpha1.ListenerRuleList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ListenerRule")
			return nil
		}
