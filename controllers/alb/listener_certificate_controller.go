...
// ListenerCertificateReconciler reconciles a ListenerCertificate object
type ListenerCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=listenercertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=listenercertificates/status,verbs=get;update;patch

func (r *ListenerCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("listenercertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ListenerCertificate")
...
func (r *ListenerCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &albv1alpha1.ListenerCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", albv1alpha1.ListenerCertificate{}.APIVersion, albv1alpha1.ListenerCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&albv1alpha1.ListenerCertificate{}).
