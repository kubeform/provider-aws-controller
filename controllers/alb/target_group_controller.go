...
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=targetgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alb.aws.kubeform.com,resources=targetgroups/status,verbs=get;update;patch

func (r *TargetGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("targetgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TargetGroup")
...
func (r *TargetGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &albv1alpha1.TargetGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", albv1alpha1.TargetGroup{}.APIVersion, albv1alpha1.TargetGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&albv1alpha1.TargetGroup{}).
