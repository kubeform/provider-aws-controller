...
// +kubebuilder:rbac:groups=codedeploy.aws.kubeform.com,resources=deploymentgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=codedeploy.aws.kubeform.com,resources=deploymentgroups/status,verbs=get;update;patch

func (r *DeploymentGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deploymentgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeploymentGroup")
...
func (r *DeploymentGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &codedeployv1alpha1.DeploymentGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", codedeployv1alpha1.DeploymentGroup{}.APIVersion, codedeployv1alpha1.DeploymentGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&codedeployv1alpha1.DeploymentGroup{}).
