...
// +kubebuilder:rbac:groups=kinesisanalyticsv2.aws.kubeform.com,resources=applicationsnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kinesisanalyticsv2.aws.kubeform.com,resources=applicationsnapshots/status,verbs=get;update;patch

func (r *ApplicationSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("applicationsnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApplicationSnapshot")
...
func (r *ApplicationSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kinesisanalyticsv2v1alpha1.ApplicationSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", kinesisanalyticsv2v1alpha1.ApplicationSnapshot{}.APIVersion, kinesisanalyticsv2v1alpha1.ApplicationSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kinesisanalyticsv2v1alpha1.ApplicationSnapshot{}).
