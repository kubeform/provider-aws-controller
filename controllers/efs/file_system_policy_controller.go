...
// FileSystemPolicyReconciler reconciles a FileSystemPolicy object
type FileSystemPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=efs.aws.kubeform.com,resources=filesystempolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=efs.aws.kubeform.com,resources=filesystempolicies/status,verbs=get;update;patch

func (r *FileSystemPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("filesystempolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FileSystemPolicy")
...
func (r *FileSystemPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &efsv1alpha1.FileSystemPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", efsv1alpha1.FileSystemPolicy{}.APIVersion, efsv1alpha1.FileSystemPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&efsv1alpha1.FileSystemPolicy{}).
