...
// AnalyzerReconciler reconciles a Analyzer object
...
// +kubebuilder:rbac:groups=accessanalyzer.aws.kubeform.com,resources=analyzers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=accessanalyzer.aws.kubeform.com,resources=analyzers/status,verbs=get;update;patch

func (r *AnalyzerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("analyzer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Analyzer")
...
func (r *AnalyzerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &accessanalyzerv1alpha1.Analyzer{}); err != nil {
			klog.Error(err, "unable to set up auditor", accessanalyzerv1alpha1.Analyzer{}.APIVersion, accessanalyzerv1alpha1.Analyzer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&accessanalyzerv1alpha1.Analyzer{}).
