...
// +kubebuilder:rbac:groups=transfer.aws.kubeform.com,resources=servers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=transfer.aws.kubeform.com,resources=servers/status,verbs=get;update;patch
...
func (r *ServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &transferv1alpha1.Server{}); err != nil {
			klog.Error(err, "unable to set up auditor", transferv1alpha1.Server{}.APIVersion, transferv1alpha1.Server{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&transferv1alpha1.Server{}).
...
func (r *ServerReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Server")
			return nil
		}
		...
		resourceList := &transferv1alpha1.ServerList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Server")
			return nil
		}
