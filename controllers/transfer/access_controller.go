...
// +kubebuilder:rbac:groups=transfer.aws.kubeform.com,resources=accesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=transfer.aws.kubeform.com,resources=accesses/status,verbs=get;update;patch
...
func (r *AccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Access")
...
func (r *AccessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &transferv1alpha1.Access{}); err != nil {
			klog.Error(err, "unable to set up auditor", transferv1alpha1.Access{}.APIVersion, transferv1alpha1.Access{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&transferv1alpha1.Access{}).
