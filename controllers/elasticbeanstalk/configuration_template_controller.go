...
// +kubebuilder:rbac:groups=elasticbeanstalk.aws.kubeform.com,resources=configurationtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elasticbeanstalk.aws.kubeform.com,resources=configurationtemplates/status,verbs=get;update;patch

func (r *ConfigurationTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configurationtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigurationTemplate")
...
func (r *ConfigurationTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elasticbeanstalkv1alpha1.ConfigurationTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", elasticbeanstalkv1alpha1.ConfigurationTemplate{}.APIVersion, elasticbeanstalkv1alpha1.ConfigurationTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&elasticbeanstalkv1alpha1.ConfigurationTemplate{}).
