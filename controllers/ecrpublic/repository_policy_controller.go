...
// +kubebuilder:rbac:groups=ecrpublic.aws.kubeform.com,resources=repositorypolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecrpublic.aws.kubeform.com,resources=repositorypolicies/status,verbs=get;update;patch

func (r *RepositoryPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("repositorypolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RepositoryPolicy")
...
func (r *RepositoryPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecrpublicv1alpha1.RepositoryPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecrpublicv1alpha1.RepositoryPolicy{}.APIVersion, ecrpublicv1alpha1.RepositoryPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecrpublicv1alpha1.RepositoryPolicy{}).
