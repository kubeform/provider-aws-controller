...
// VpcDHCPOptionsReconciler reconciles a VpcDHCPOptions object
type VpcDHCPOptionsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=default.aws.kubeform.com,resources=vpcdhcpoptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=default.aws.kubeform.com,resources=vpcdhcpoptions/status,verbs=get;update;patch

func (r *VpcDHCPOptionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcdhcpoptions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcDHCPOptions")
...
func (r *VpcDHCPOptionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &defaultv1alpha1.VpcDHCPOptions{}); err != nil {
			klog.Error(err, "unable to set up auditor", defaultv1alpha1.VpcDHCPOptions{}.APIVersion, defaultv1alpha1.VpcDHCPOptions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&defaultv1alpha1.VpcDHCPOptions{}).
