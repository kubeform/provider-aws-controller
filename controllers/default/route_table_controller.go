...
// +kubebuilder:rbac:groups=default.aws.kubeform.com,resources=routetables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=default.aws.kubeform.com,resources=routetables/status,verbs=get;update;patch

func (r *RouteTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routetable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RouteTable")
...
func (r *RouteTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &defaultv1alpha1.RouteTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", defaultv1alpha1.RouteTable{}.APIVersion, defaultv1alpha1.RouteTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&defaultv1alpha1.RouteTable{}).
