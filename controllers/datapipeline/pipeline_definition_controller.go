...
// +kubebuilder:rbac:groups=datapipeline.aws.kubeform.com,resources=pipelinedefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datapipeline.aws.kubeform.com,resources=pipelinedefinitions/status,verbs=get;update;patch

func (r *PipelineDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pipelinedefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PipelineDefinition")
...
func (r *PipelineDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datapipelinev1alpha1.PipelineDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", datapipelinev1alpha1.PipelineDefinition{}.APIVersion, datapipelinev1alpha1.PipelineDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datapipelinev1alpha1.PipelineDefinition{}).
