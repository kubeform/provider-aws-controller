...
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=networkprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=networkprofiles/status,verbs=get;update;patch

func (r *NetworkProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networkprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkProfile")
...
func (r *NetworkProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devicefarmv1alpha1.NetworkProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", devicefarmv1alpha1.NetworkProfile{}.APIVersion, devicefarmv1alpha1.NetworkProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devicefarmv1alpha1.NetworkProfile{}).
