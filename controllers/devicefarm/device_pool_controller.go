...
// DevicePoolReconciler reconciles a DevicePool object
...
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=devicepools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=devicepools/status,verbs=get;update;patch

func (r *DevicePoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("devicepool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DevicePool")
...
func (r *DevicePoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devicefarmv1alpha1.DevicePool{}); err != nil {
			klog.Error(err, "unable to set up auditor", devicefarmv1alpha1.DevicePool{}.APIVersion, devicefarmv1alpha1.DevicePool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devicefarmv1alpha1.DevicePool{}).
