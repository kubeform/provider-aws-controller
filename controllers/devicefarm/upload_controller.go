...
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=uploads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=uploads/status,verbs=get;update;patch

func (r *UploadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("upload", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Upload")
...
func (r *UploadReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devicefarmv1alpha1.Upload{}); err != nil {
			klog.Error(err, "unable to set up auditor", devicefarmv1alpha1.Upload{}.APIVersion, devicefarmv1alpha1.Upload{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devicefarmv1alpha1.Upload{}).
