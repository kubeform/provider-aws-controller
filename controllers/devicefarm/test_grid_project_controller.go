...
// TestGridProjectReconciler reconciles a TestGridProject object
type TestGridProjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=testgridprojects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=devicefarm.aws.kubeform.com,resources=testgridprojects/status,verbs=get;update;patch

func (r *TestGridProjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("testgridproject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TestGridProject")
...
func (r *TestGridProjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devicefarmv1alpha1.TestGridProject{}); err != nil {
			klog.Error(err, "unable to set up auditor", devicefarmv1alpha1.TestGridProject{}.APIVersion, devicefarmv1alpha1.TestGridProject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devicefarmv1alpha1.TestGridProject{}).
