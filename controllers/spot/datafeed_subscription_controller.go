...
// DatafeedSubscriptionReconciler reconciles a DatafeedSubscription object
type DatafeedSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spot.aws.kubeform.com,resources=datafeedsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spot.aws.kubeform.com,resources=datafeedsubscriptions/status,verbs=get;update;patch

func (r *DatafeedSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datafeedsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DatafeedSubscription")
...
func (r *DatafeedSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spotv1alpha1.DatafeedSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", spotv1alpha1.DatafeedSubscription{}.APIVersion, spotv1alpha1.DatafeedSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spotv1alpha1.DatafeedSubscription{}).
