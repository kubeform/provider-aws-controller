...
// FleetRequestReconciler reconciles a FleetRequest object
type FleetRequestReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spot.aws.kubeform.com,resources=fleetrequests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spot.aws.kubeform.com,resources=fleetrequests/status,verbs=get;update;patch

func (r *FleetRequestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fleetrequest", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FleetRequest")
...
func (r *FleetRequestReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spotv1alpha1.FleetRequest{}); err != nil {
			klog.Error(err, "unable to set up auditor", spotv1alpha1.FleetRequest{}.APIVersion, spotv1alpha1.FleetRequest{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spotv1alpha1.FleetRequest{}).
