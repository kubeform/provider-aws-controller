...
// AdminAccountReconciler reconciles a AdminAccount object
type AdminAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fms.aws.kubeform.com,resources=adminaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fms.aws.kubeform.com,resources=adminaccounts/status,verbs=get;update;patch

func (r *AdminAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("adminaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AdminAccount")
...
func (r *AdminAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fmsv1alpha1.AdminAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", fmsv1alpha1.AdminAccount{}.APIVersion, fmsv1alpha1.AdminAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fmsv1alpha1.AdminAccount{}).
