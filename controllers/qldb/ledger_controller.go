...
// +kubebuilder:rbac:groups=qldb.aws.kubeform.com,resources=ledgers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=qldb.aws.kubeform.com,resources=ledgers/status,verbs=get;update;patch

func (r *LedgerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ledger", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ledger")
...
func (r *LedgerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &qldbv1alpha1.Ledger{}); err != nil {
			klog.Error(err, "unable to set up auditor", qldbv1alpha1.Ledger{}.APIVersion, qldbv1alpha1.Ledger{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&qldbv1alpha1.Ledger{}).
