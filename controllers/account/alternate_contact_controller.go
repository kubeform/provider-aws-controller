...
// AlternateContactReconciler reconciles a AlternateContact object
type AlternateContactReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=account.aws.kubeform.com,resources=alternatecontacts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=account.aws.kubeform.com,resources=alternatecontacts/status,verbs=get;update;patch

func (r *AlternateContactReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alternatecontact", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlternateContact")
...
func (r *AlternateContactReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &accountv1alpha1.AlternateContact{}); err != nil {
			klog.Error(err, "unable to set up auditor", accountv1alpha1.AlternateContact{}.APIVersion, accountv1alpha1.AlternateContact{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&accountv1alpha1.AlternateContact{}).
