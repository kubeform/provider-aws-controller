...
// BackendServerPolicyReconciler reconciles a BackendServerPolicy object
type BackendServerPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=loadbalancer.aws.kubeform.com,resources=backendserverpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=loadbalancer.aws.kubeform.com,resources=backendserverpolicies/status,verbs=get;update;patch

func (r *BackendServerPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendserverpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendServerPolicy")
...
func (r *BackendServerPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &loadbalancerv1alpha1.BackendServerPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", loadbalancerv1alpha1.BackendServerPolicy{}.APIVersion, loadbalancerv1alpha1.BackendServerPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&loadbalancerv1alpha1.BackendServerPolicy{}).
