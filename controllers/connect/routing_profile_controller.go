...
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=routingprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=routingprofiles/status,verbs=get;update;patch

func (r *RoutingProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routingprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RoutingProfile")
...
func (r *RoutingProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &connectv1alpha1.RoutingProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", connectv1alpha1.RoutingProfile{}.APIVersion, connectv1alpha1.RoutingProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&connectv1alpha1.RoutingProfile{}).
