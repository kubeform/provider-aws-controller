...
// QuickConnectReconciler reconciles a QuickConnect object
type QuickConnectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=quickconnects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=quickconnects/status,verbs=get;update;patch

func (r *QuickConnectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("quickconnect", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QuickConnect")
...
func (r *QuickConnectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &connectv1alpha1.QuickConnect{}); err != nil {
			klog.Error(err, "unable to set up auditor", connectv1alpha1.QuickConnect{}.APIVersion, connectv1alpha1.QuickConnect{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&connectv1alpha1.QuickConnect{}).
