...
// SecurityProfileReconciler reconciles a SecurityProfile object
...
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=securityprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=securityprofiles/status,verbs=get;update;patch

func (r *SecurityProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("securityprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecurityProfile")
...
func (r *SecurityProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &connectv1alpha1.SecurityProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", connectv1alpha1.SecurityProfile{}.APIVersion, connectv1alpha1.SecurityProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&connectv1alpha1.SecurityProfile{}).
