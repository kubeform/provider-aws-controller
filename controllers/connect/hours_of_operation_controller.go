...
// HoursOfOperationReconciler reconciles a HoursOfOperation object
type HoursOfOperationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=hoursofoperations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=connect.aws.kubeform.com,resources=hoursofoperations/status,verbs=get;update;patch

func (r *HoursOfOperationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hoursofoperation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HoursOfOperation")
...
func (r *HoursOfOperationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &connectv1alpha1.HoursOfOperation{}); err != nil {
			klog.Error(err, "unable to set up auditor", connectv1alpha1.HoursOfOperation{}.APIVersion, connectv1alpha1.HoursOfOperation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&connectv1alpha1.HoursOfOperation{}).
