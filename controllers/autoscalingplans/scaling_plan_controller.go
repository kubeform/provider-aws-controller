...
// ScalingPlanReconciler reconciles a ScalingPlan object
type ScalingPlanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=autoscalingplans.aws.kubeform.com,resources=scalingplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=autoscalingplans.aws.kubeform.com,resources=scalingplans/status,verbs=get;update;patch

func (r *ScalingPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scalingplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScalingPlan")
...
func (r *ScalingPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &autoscalingplansv1alpha1.ScalingPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", autoscalingplansv1alpha1.ScalingPlan{}.APIVersion, autoscalingplansv1alpha1.ScalingPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&autoscalingplansv1alpha1.ScalingPlan{}).
