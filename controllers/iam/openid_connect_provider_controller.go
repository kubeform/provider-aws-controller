...
// OpenidConnectProviderReconciler reconciles a OpenidConnectProvider object
type OpenidConnectProviderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=openidconnectproviders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=openidconnectproviders/status,verbs=get;update;patch

func (r *OpenidConnectProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("openidconnectprovider", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OpenidConnectProvider")
...
func (r *OpenidConnectProviderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.OpenidConnectProvider{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.OpenidConnectProvider{}.APIVersion, iamv1alpha1.OpenidConnectProvider{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.OpenidConnectProvider{}).
