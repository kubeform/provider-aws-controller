...
// ServiceSpecificCredentialReconciler reconciles a ServiceSpecificCredential object
type ServiceSpecificCredentialReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=servicespecificcredentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=servicespecificcredentials/status,verbs=get;update;patch

func (r *ServiceSpecificCredentialReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicespecificcredential", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceSpecificCredential")
...
func (r *ServiceSpecificCredentialReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.ServiceSpecificCredential{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.ServiceSpecificCredential{}.APIVersion, iamv1alpha1.ServiceSpecificCredential{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.ServiceSpecificCredential{}).
...
func (r *ServiceSpecificCredentialReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ServiceSpecificCredential")
			return nil
		}
		...
		resourceList := &iamv1alpha1.ServiceSpecificCredentialList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ServiceSpecificCredential")
			return nil
		}
