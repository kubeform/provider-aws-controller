...
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=grouppolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=grouppolicies/status,verbs=get;update;patch

func (r *GroupPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("grouppolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupPolicy")
...
func (r *GroupPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.GroupPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.GroupPolicy{}.APIVersion, iamv1alpha1.GroupPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.GroupPolicy{}).
