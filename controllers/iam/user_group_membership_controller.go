...
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=usergroupmemberships,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=usergroupmemberships/status,verbs=get;update;patch

func (r *UserGroupMembershipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("usergroupmembership", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserGroupMembership")
...
func (r *UserGroupMembershipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.UserGroupMembership{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.UserGroupMembership{}.APIVersion, iamv1alpha1.UserGroupMembership{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.UserGroupMembership{}).
