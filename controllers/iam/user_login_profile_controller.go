...
// UserLoginProfileReconciler reconciles a UserLoginProfile object
type UserLoginProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=userloginprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iam.aws.kubeform.com,resources=userloginprofiles/status,verbs=get;update;patch

func (r *UserLoginProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userloginprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserLoginProfile")
...
func (r *UserLoginProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iamv1alpha1.UserLoginProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", iamv1alpha1.UserLoginProfile{}.APIVersion, iamv1alpha1.UserLoginProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iamv1alpha1.UserLoginProfile{}).
