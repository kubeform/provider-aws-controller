...
// CatalogTableReconciler reconciles a CatalogTable object
type CatalogTableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=catalogtables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=catalogtables/status,verbs=get;update;patch

func (r *CatalogTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogtable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogTable")
...
func (r *CatalogTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gluev1alpha1.CatalogTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", gluev1alpha1.CatalogTable{}.APIVersion, gluev1alpha1.CatalogTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gluev1alpha1.CatalogTable{}).
