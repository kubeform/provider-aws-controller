...
// MlTransformReconciler reconciles a MlTransform object
type MlTransformReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=mltransforms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=mltransforms/status,verbs=get;update;patch

func (r *MlTransformReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mltransform", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MlTransform")
...
func (r *MlTransformReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gluev1alpha1.MlTransform{}); err != nil {
			klog.Error(err, "unable to set up auditor", gluev1alpha1.MlTransform{}.APIVersion, gluev1alpha1.MlTransform{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gluev1alpha1.MlTransform{}).
