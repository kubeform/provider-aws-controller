...
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=partitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=partitions/status,verbs=get;update;patch
...
func (r *PartitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Partition")
...
func (r *PartitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gluev1alpha1.Partition{}); err != nil {
			klog.Error(err, "unable to set up auditor", gluev1alpha1.Partition{}.APIVersion, gluev1alpha1.Partition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gluev1alpha1.Partition{}).
