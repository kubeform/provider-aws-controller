...
// PartitionIndexReconciler reconciles a PartitionIndex object
type PartitionIndexReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=partitionindices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=partitionindices/status,verbs=get;update;patch

func (r *PartitionIndexReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("partitionindex", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PartitionIndex")
...
func (r *PartitionIndexReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gluev1alpha1.PartitionIndex{}); err != nil {
			klog.Error(err, "unable to set up auditor", gluev1alpha1.PartitionIndex{}.APIVersion, gluev1alpha1.PartitionIndex{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gluev1alpha1.PartitionIndex{}).
