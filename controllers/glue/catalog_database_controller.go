...
// CatalogDatabaseReconciler reconciles a CatalogDatabase object
type CatalogDatabaseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=catalogdatabases,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.aws.kubeform.com,resources=catalogdatabases/status,verbs=get;update;patch

func (r *CatalogDatabaseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("catalogdatabase", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CatalogDatabase")
...
func (r *CatalogDatabaseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gluev1alpha1.CatalogDatabase{}); err != nil {
			klog.Error(err, "unable to set up auditor", gluev1alpha1.CatalogDatabase{}.APIVersion, gluev1alpha1.CatalogDatabase{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gluev1alpha1.CatalogDatabase{}).
