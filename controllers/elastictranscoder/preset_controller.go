...
// +kubebuilder:rbac:groups=elastictranscoder.aws.kubeform.com,resources=presets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elastictranscoder.aws.kubeform.com,resources=presets/status,verbs=get;update;patch

func (r *PresetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("preset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Preset")
...
func (r *PresetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elastictranscoderv1alpha1.Preset{}); err != nil {
			klog.Error(err, "unable to set up auditor", elastictranscoderv1alpha1.Preset{}.APIVersion, elastictranscoderv1alpha1.Preset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&elastictranscoderv1alpha1.Preset{}).
