...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=documentationparts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=documentationparts/status,verbs=get;update;patch

func (r *DocumentationPartReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DocumentationPart")
...
func (r *DocumentationPartReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.DocumentationPart{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.DocumentationPart{}.APIVersion, apigatewayv1alpha1.DocumentationPart{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.DocumentationPart{}).
