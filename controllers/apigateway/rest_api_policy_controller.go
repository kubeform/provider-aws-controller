...
// RestAPIPolicyReconciler reconciles a RestAPIPolicy object
type RestAPIPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=restapipolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=restapipolicies/status,verbs=get;update;patch

func (r *RestAPIPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("restapipolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RestAPIPolicy")
...
func (r *RestAPIPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.RestAPIPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.RestAPIPolicy{}.APIVersion, apigatewayv1alpha1.RestAPIPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.RestAPIPolicy{}).
