...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=basepathmappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=basepathmappings/status,verbs=get;update;patch

func (r *BasePathMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BasePathMapping")
...
func (r *BasePathMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.BasePathMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.BasePathMapping{}.APIVersion, apigatewayv1alpha1.BasePathMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.BasePathMapping{}).
