...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=requestvalidators,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=requestvalidators/status,verbs=get;update;patch

func (r *RequestValidatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RequestValidator")
...
func (r *RequestValidatorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.RequestValidator{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.RequestValidator{}.APIVersion, apigatewayv1alpha1.RequestValidator{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.RequestValidator{}).
