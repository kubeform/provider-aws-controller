...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=usageplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=usageplans/status,verbs=get;update;patch

func (r *UsagePlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UsagePlan")
...
func (r *UsagePlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.UsagePlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.UsagePlan{}.APIVersion, apigatewayv1alpha1.UsagePlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.UsagePlan{}).
