...
// ApigatewayResourceReconciler reconciles a ApigatewayResource object
type ApigatewayResourceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=apigatewayresources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=apigatewayresources/status,verbs=get;update;patch

func (r *ApigatewayResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apigatewayresource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApigatewayResource")
...
func (r *ApigatewayResourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.ApigatewayResource{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.ApigatewayResource{}.APIVersion, apigatewayv1alpha1.ApigatewayResource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.ApigatewayResource{}).
