...
// MethodSettingsReconciler reconciles a MethodSettings object
type MethodSettingsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=methodsettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=methodsettings/status,verbs=get;update;patch

func (r *MethodSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("methodsettings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MethodSettings")
...
func (r *MethodSettingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.MethodSettings{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.MethodSettings{}.APIVersion, apigatewayv1alpha1.MethodSettings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.MethodSettings{}).
