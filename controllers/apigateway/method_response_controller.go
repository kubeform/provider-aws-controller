...
// MethodResponseReconciler reconciles a MethodResponse object
type MethodResponseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=methodresponses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=methodresponses/status,verbs=get;update;patch

func (r *MethodResponseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("methodresponse", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MethodResponse")
...
func (r *MethodResponseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.MethodResponse{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.MethodResponse{}.APIVersion, apigatewayv1alpha1.MethodResponse{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.MethodResponse{}).
