...
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=usageplankeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigateway.aws.kubeform.com,resources=usageplankeys/status,verbs=get;update;patch

func (r *UsagePlanKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UsagePlanKey")
...
func (r *UsagePlanKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv1alpha1.UsagePlanKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv1alpha1.UsagePlanKey{}.APIVersion, apigatewayv1alpha1.UsagePlanKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv1alpha1.UsagePlanKey{}).
