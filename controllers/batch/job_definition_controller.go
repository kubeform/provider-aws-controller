...
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=jobdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=jobdefinitions/status,verbs=get;update;patch

func (r *JobDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("jobdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobDefinition")
...
func (r *JobDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &batchv1alpha1.JobDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", batchv1alpha1.JobDefinition{}.APIVersion, batchv1alpha1.JobDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&batchv1alpha1.JobDefinition{}).
