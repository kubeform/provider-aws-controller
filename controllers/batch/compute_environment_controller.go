...
// ComputeEnvironmentReconciler reconciles a ComputeEnvironment object
type ComputeEnvironmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=computeenvironments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=computeenvironments/status,verbs=get;update;patch

func (r *ComputeEnvironmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("computeenvironment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ComputeEnvironment")
...
func (r *ComputeEnvironmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &batchv1alpha1.ComputeEnvironment{}); err != nil {
			klog.Error(err, "unable to set up auditor", batchv1alpha1.ComputeEnvironment{}.APIVersion, batchv1alpha1.ComputeEnvironment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&batchv1alpha1.ComputeEnvironment{}).
