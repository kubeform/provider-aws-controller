...
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=jobqueues,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=jobqueues/status,verbs=get;update;patch
...
func (r *JobQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch JobQueue")
...
func (r *JobQueueReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &batchv1alpha1.JobQueue{}); err != nil {
			klog.Error(err, "unable to set up auditor", batchv1alpha1.JobQueue{}.APIVersion, batchv1alpha1.JobQueue{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&batchv1alpha1.JobQueue{}).
