...
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=schedulingpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch.aws.kubeform.com,resources=schedulingpolicies/status,verbs=get;update;patch

func (r *SchedulingPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("schedulingpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SchedulingPolicy")
...
func (r *SchedulingPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &batchv1alpha1.SchedulingPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", batchv1alpha1.SchedulingPolicy{}.APIVersion, batchv1alpha1.SchedulingPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&batchv1alpha1.SchedulingPolicy{}).
