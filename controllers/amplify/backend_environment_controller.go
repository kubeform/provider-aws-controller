...
// BackendEnvironmentReconciler reconciles a BackendEnvironment object
type BackendEnvironmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=backendenvironments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=backendenvironments/status,verbs=get;update;patch

func (r *BackendEnvironmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("backendenvironment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BackendEnvironment")
...
func (r *BackendEnvironmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amplifyv1alpha1.BackendEnvironment{}); err != nil {
			klog.Error(err, "unable to set up auditor", amplifyv1alpha1.BackendEnvironment{}.APIVersion, amplifyv1alpha1.BackendEnvironment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amplifyv1alpha1.BackendEnvironment{}).
