...
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=branches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=branches/status,verbs=get;update;patch

func (r *BranchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("branch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Branch")
...
func (r *BranchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amplifyv1alpha1.Branch{}); err != nil {
			klog.Error(err, "unable to set up auditor", amplifyv1alpha1.Branch{}.APIVersion, amplifyv1alpha1.Branch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amplifyv1alpha1.Branch{}).
...
func (r *BranchReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Branch")
			return nil
		}
		...
		resourceList := &amplifyv1alpha1.BranchList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Branch")
			return nil
		}
