...
// DomainAssociationReconciler reconciles a DomainAssociation object
type DomainAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=domainassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=amplify.aws.kubeform.com,resources=domainassociations/status,verbs=get;update;patch

func (r *DomainAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainAssociation")
...
func (r *DomainAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amplifyv1alpha1.DomainAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", amplifyv1alpha1.DomainAssociation{}.APIVersion, amplifyv1alpha1.DomainAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amplifyv1alpha1.DomainAssociation{}).
