...
// GlobalReplicationGroupReconciler reconciles a GlobalReplicationGroup object
type GlobalReplicationGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=elasticache.aws.kubeform.com,resources=globalreplicationgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elasticache.aws.kubeform.com,resources=globalreplicationgroups/status,verbs=get;update;patch

func (r *GlobalReplicationGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globalreplicationgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalReplicationGroup")
...
func (r *GlobalReplicationGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elasticachev1alpha1.GlobalReplicationGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", elasticachev1alpha1.GlobalReplicationGroup{}.APIVersion, elasticachev1alpha1.GlobalReplicationGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&elasticachev1alpha1.GlobalReplicationGroup{}).
