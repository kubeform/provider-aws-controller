...
// +kubebuilder:rbac:groups=elasticache.aws.kubeform.com,resources=replicationgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elasticache.aws.kubeform.com,resources=replicationgroups/status,verbs=get;update;patch

func (r *ReplicationGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicationgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicationGroup")
...
func (r *ReplicationGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elasticachev1alpha1.ReplicationGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", elasticachev1alpha1.ReplicationGroup{}.APIVersion, elasticachev1alpha1.ReplicationGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&elasticachev1alpha1.ReplicationGroup{}).
...
func (r *ReplicationGroupReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ReplicationGroup")
			return nil
		}
		...
		resourceList := &elasticachev1alpha1.ReplicationGroupList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ReplicationGroup")
			return nil
		}
