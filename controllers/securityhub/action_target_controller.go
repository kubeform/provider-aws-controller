...
// ActionTargetReconciler reconciles a ActionTarget object
...
// +kubebuilder:rbac:groups=securityhub.aws.kubeform.com,resources=actiontargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=securityhub.aws.kubeform.com,resources=actiontargets/status,verbs=get;update;patch

func (r *ActionTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("actiontarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ActionTarget")
...
func (r *ActionTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityhubv1alpha1.ActionTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityhubv1alpha1.ActionTarget{}.APIVersion, securityhubv1alpha1.ActionTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityhubv1alpha1.ActionTarget{}).
