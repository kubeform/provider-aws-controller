...
// ProductSubscriptionReconciler reconciles a ProductSubscription object
type ProductSubscriptionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=securityhub.aws.kubeform.com,resources=productsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=securityhub.aws.kubeform.com,resources=productsubscriptions/status,verbs=get;update;patch

func (r *ProductSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("productsubscription", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProductSubscription")
...
func (r *ProductSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityhubv1alpha1.ProductSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityhubv1alpha1.ProductSubscription{}.APIVersion, securityhubv1alpha1.ProductSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityhubv1alpha1.ProductSubscription{}).
