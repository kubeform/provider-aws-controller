...
// SamlOptionsReconciler reconciles a SamlOptions object
type SamlOptionsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=elasticsearchdomain.aws.kubeform.com,resources=samloptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elasticsearchdomain.aws.kubeform.com,resources=samloptions/status,verbs=get;update;patch

func (r *SamlOptionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("samloptions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SamlOptions")
...
func (r *SamlOptionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elasticsearchdomainv1alpha1.SamlOptions{}); err != nil {
			klog.Error(err, "unable to set up auditor", elasticsearchdomainv1alpha1.SamlOptions{}.APIVersion, elasticsearchdomainv1alpha1.SamlOptions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&elasticsearchdomainv1alpha1.SamlOptions{}).
...
func (r *SamlOptionsReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SamlOptions")
			return nil
		}
		...
		resourceList := &elasticsearchdomainv1alpha1.SamlOptionsList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SamlOptions")
			return nil
		}
