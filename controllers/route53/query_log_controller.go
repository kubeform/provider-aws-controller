...
// QueryLogReconciler reconciles a QueryLog object
type QueryLogReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=querylogs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=querylogs/status,verbs=get;update;patch

func (r *QueryLogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("querylog", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch QueryLog")
...
func (r *QueryLogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53v1alpha1.QueryLog{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53v1alpha1.QueryLog{}.APIVersion, route53v1alpha1.QueryLog{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53v1alpha1.QueryLog{}).
