...
// HostedZoneDnssecReconciler reconciles a HostedZoneDnssec object
type HostedZoneDnssecReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=hostedzonednssecs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=hostedzonednssecs/status,verbs=get;update;patch

func (r *HostedZoneDnssecReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostedzonednssec", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostedZoneDnssec")
...
func (r *HostedZoneDnssecReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53v1alpha1.HostedZoneDnssec{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53v1alpha1.HostedZoneDnssec{}.APIVersion, route53v1alpha1.HostedZoneDnssec{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53v1alpha1.HostedZoneDnssec{}).
