...
// KeySigningKeyReconciler reconciles a KeySigningKey object
type KeySigningKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=keysigningkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=keysigningkeys/status,verbs=get;update;patch

func (r *KeySigningKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("keysigningkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KeySigningKey")
...
func (r *KeySigningKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53v1alpha1.KeySigningKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53v1alpha1.KeySigningKey{}.APIVersion, route53v1alpha1.KeySigningKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53v1alpha1.KeySigningKey{}).
