...
// ResolverRuleReconciler reconciles a ResolverRule object
...
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=resolverrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=resolverrules/status,verbs=get;update;patch

func (r *ResolverRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resolverrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResolverRule")
...
func (r *ResolverRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53v1alpha1.ResolverRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53v1alpha1.ResolverRule{}.APIVersion, route53v1alpha1.ResolverRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53v1alpha1.ResolverRule{}).
