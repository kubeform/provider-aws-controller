...
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=resolverendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53.aws.kubeform.com,resources=resolverendpoints/status,verbs=get;update;patch

func (r *ResolverEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resolverendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResolverEndpoint")
...
func (r *ResolverEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53v1alpha1.ResolverEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53v1alpha1.ResolverEndpoint{}.APIVersion, route53v1alpha1.ResolverEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53v1alpha1.ResolverEndpoint{}).
