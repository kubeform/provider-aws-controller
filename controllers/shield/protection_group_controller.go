...
// ProtectionGroupReconciler reconciles a ProtectionGroup object
type ProtectionGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=shield.aws.kubeform.com,resources=protectiongroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=shield.aws.kubeform.com,resources=protectiongroups/status,verbs=get;update;patch

func (r *ProtectionGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protectiongroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProtectionGroup")
...
func (r *ProtectionGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &shieldv1alpha1.ProtectionGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", shieldv1alpha1.ProtectionGroup{}.APIVersion, shieldv1alpha1.ProtectionGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&shieldv1alpha1.ProtectionGroup{}).
