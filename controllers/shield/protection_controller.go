...
// +kubebuilder:rbac:groups=shield.aws.kubeform.com,resources=protections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=shield.aws.kubeform.com,resources=protections/status,verbs=get;update;patch

func (r *ProtectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Protection")
...
func (r *ProtectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &shieldv1alpha1.Protection{}); err != nil {
			klog.Error(err, "unable to set up auditor", shieldv1alpha1.Protection{}.APIVersion, shieldv1alpha1.Protection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&shieldv1alpha1.Protection{}).
