...
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=documents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=documents/status,verbs=get;update;patch

func (r *DocumentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("document", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Document")
...
func (r *DocumentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ssmv1alpha1.Document{}); err != nil {
			klog.Error(err, "unable to set up auditor", ssmv1alpha1.Document{}.APIVersion, ssmv1alpha1.Document{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ssmv1alpha1.Document{}).
