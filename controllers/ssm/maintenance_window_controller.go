...
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=maintenancewindows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=maintenancewindows/status,verbs=get;update;patch

func (r *MaintenanceWindowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("maintenancewindow", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MaintenanceWindow")
...
func (r *MaintenanceWindowReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ssmv1alpha1.MaintenanceWindow{}); err != nil {
			klog.Error(err, "unable to set up auditor", ssmv1alpha1.MaintenanceWindow{}.APIVersion, ssmv1alpha1.MaintenanceWindow{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ssmv1alpha1.MaintenanceWindow{}).
