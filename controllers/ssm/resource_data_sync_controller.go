...
// ResourceDataSyncReconciler reconciles a ResourceDataSync object
type ResourceDataSyncReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=resourcedatasyncs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ssm.aws.kubeform.com,resources=resourcedatasyncs/status,verbs=get;update;patch

func (r *ResourceDataSyncReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourcedatasync", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceDataSync")
...
func (r *ResourceDataSyncReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ssmv1alpha1.ResourceDataSync{}); err != nil {
			klog.Error(err, "unable to set up auditor", ssmv1alpha1.ResourceDataSync{}.APIVersion, ssmv1alpha1.ResourceDataSync{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ssmv1alpha1.ResourceDataSync{}).
