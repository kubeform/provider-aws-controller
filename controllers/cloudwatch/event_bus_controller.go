...
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=eventbus,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=eventbus/status,verbs=get;update;patch
...
func (r *EventBusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventBus")
...
func (r *EventBusReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudwatchv1alpha1.EventBus{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudwatchv1alpha1.EventBus{}.APIVersion, cloudwatchv1alpha1.EventBus{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudwatchv1alpha1.EventBus{}).
