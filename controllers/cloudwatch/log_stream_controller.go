...
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=logstreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=logstreams/status,verbs=get;update;patch

func (r *LogStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("logstream", req.NamespacedName)
...
func (r *LogStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudwatchv1alpha1.LogStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudwatchv1alpha1.LogStream{}.APIVersion, cloudwatchv1alpha1.LogStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudwatchv1alpha1.LogStream{}).
