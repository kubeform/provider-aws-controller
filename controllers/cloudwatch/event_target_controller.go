...
// EventTargetReconciler reconciles a EventTarget object
...
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=eventtargets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=eventtargets/status,verbs=get;update;patch

func (r *EventTargetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eventtarget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventTarget")
...
func (r *EventTargetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudwatchv1alpha1.EventTarget{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudwatchv1alpha1.EventTarget{}.APIVersion, cloudwatchv1alpha1.EventTarget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudwatchv1alpha1.EventTarget{}).
