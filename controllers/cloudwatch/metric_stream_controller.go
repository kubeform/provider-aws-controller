...
// MetricStreamReconciler reconciles a MetricStream object
type MetricStreamReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=metricstreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudwatch.aws.kubeform.com,resources=metricstreams/status,verbs=get;update;patch

func (r *MetricStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metricstream", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MetricStream")
...
func (r *MetricStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudwatchv1alpha1.MetricStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudwatchv1alpha1.MetricStream{}.APIVersion, cloudwatchv1alpha1.MetricStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudwatchv1alpha1.MetricStream{}).
