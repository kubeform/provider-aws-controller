...
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=members,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=members/status,verbs=get;update;patch

func (r *MemberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("member", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Member")
...
func (r *MemberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &guarddutyv1alpha1.Member{}); err != nil {
			klog.Error(err, "unable to set up auditor", guarddutyv1alpha1.Member{}.APIVersion, guarddutyv1alpha1.Member{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&guarddutyv1alpha1.Member{}).
