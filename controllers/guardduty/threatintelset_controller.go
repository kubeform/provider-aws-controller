...
// ThreatintelsetReconciler reconciles a Threatintelset object
type ThreatintelsetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=threatintelsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=threatintelsets/status,verbs=get;update;patch

func (r *ThreatintelsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("threatintelset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Threatintelset")
...
func (r *ThreatintelsetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &guarddutyv1alpha1.Threatintelset{}); err != nil {
			klog.Error(err, "unable to set up auditor", guarddutyv1alpha1.Threatintelset{}.APIVersion, guarddutyv1alpha1.Threatintelset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&guarddutyv1alpha1.Threatintelset{}).
