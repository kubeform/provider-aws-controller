...
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=detectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=detectors/status,verbs=get;update;patch

func (r *DetectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("detector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Detector")
...
func (r *DetectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &guarddutyv1alpha1.Detector{}); err != nil {
			klog.Error(err, "unable to set up auditor", guarddutyv1alpha1.Detector{}.APIVersion, guarddutyv1alpha1.Detector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&guarddutyv1alpha1.Detector{}).
