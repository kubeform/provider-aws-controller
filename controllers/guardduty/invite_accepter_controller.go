...
// InviteAccepterReconciler reconciles a InviteAccepter object
type InviteAccepterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=inviteaccepters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=guardduty.aws.kubeform.com,resources=inviteaccepters/status,verbs=get;update;patch

func (r *InviteAccepterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("inviteaccepter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InviteAccepter")
...
func (r *InviteAccepterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &guarddutyv1alpha1.InviteAccepter{}); err != nil {
			klog.Error(err, "unable to set up auditor", guarddutyv1alpha1.InviteAccepter{}.APIVersion, guarddutyv1alpha1.InviteAccepter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&guarddutyv1alpha1.InviteAccepter{}).
