...
// NfsFileShareReconciler reconciles a NfsFileShare object
type NfsFileShareReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=nfsfileshares,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=nfsfileshares/status,verbs=get;update;patch

func (r *NfsFileShareReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("nfsfileshare", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NfsFileShare")
...
func (r *NfsFileShareReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.NfsFileShare{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.NfsFileShare{}.APIVersion, storagegatewayv1alpha1.NfsFileShare{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.NfsFileShare{}).
