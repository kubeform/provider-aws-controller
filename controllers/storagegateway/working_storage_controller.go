...
// WorkingStorageReconciler reconciles a WorkingStorage object
type WorkingStorageReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=workingstorages,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=workingstorages/status,verbs=get;update;patch

func (r *WorkingStorageReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workingstorage", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkingStorage")
...
func (r *WorkingStorageReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.WorkingStorage{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.WorkingStorage{}.APIVersion, storagegatewayv1alpha1.WorkingStorage{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.WorkingStorage{}).
