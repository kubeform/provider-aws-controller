...
// FileSystemAssociationReconciler reconciles a FileSystemAssociation object
type FileSystemAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=filesystemassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=filesystemassociations/status,verbs=get;update;patch

func (r *FileSystemAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("filesystemassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FileSystemAssociation")
...
func (r *FileSystemAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.FileSystemAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.FileSystemAssociation{}.APIVersion, storagegatewayv1alpha1.FileSystemAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.FileSystemAssociation{}).
...
func (r *FileSystemAssociationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for FileSystemAssociation")
			return nil
		}
		...
		resourceList := &storagegatewayv1alpha1.FileSystemAssociationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list FileSystemAssociation")
			return nil
		}
