...
// SmbFileShareReconciler reconciles a SmbFileShare object
type SmbFileShareReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=smbfileshares,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=smbfileshares/status,verbs=get;update;patch

func (r *SmbFileShareReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("smbfileshare", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SmbFileShare")
...
func (r *SmbFileShareReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.SmbFileShare{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.SmbFileShare{}.APIVersion, storagegatewayv1alpha1.SmbFileShare{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.SmbFileShare{}).
