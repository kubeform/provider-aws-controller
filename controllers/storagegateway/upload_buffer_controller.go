...
// UploadBufferReconciler reconciles a UploadBuffer object
type UploadBufferReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=uploadbuffers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=uploadbuffers/status,verbs=get;update;patch

func (r *UploadBufferReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("uploadbuffer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UploadBuffer")
...
func (r *UploadBufferReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.UploadBuffer{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.UploadBuffer{}.APIVersion, storagegatewayv1alpha1.UploadBuffer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.UploadBuffer{}).
