...
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=caches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storagegateway.aws.kubeform.com,resources=caches/status,verbs=get;update;patch

func (r *CacheReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cache", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Cache")
...
func (r *CacheReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagegatewayv1alpha1.Cache{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagegatewayv1alpha1.Cache{}.APIVersion, storagegatewayv1alpha1.Cache{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagegatewayv1alpha1.Cache{}).
