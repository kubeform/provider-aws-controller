...
// ImageVersionReconciler reconciles a ImageVersion object
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=imageversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=imageversions/status,verbs=get;update;patch

func (r *ImageVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("imageversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ImageVersion")
...
func (r *ImageVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.ImageVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.ImageVersion{}.APIVersion, sagemakerv1alpha1.ImageVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.ImageVersion{}).
