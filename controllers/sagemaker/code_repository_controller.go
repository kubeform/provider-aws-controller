...
// CodeRepositoryReconciler reconciles a CodeRepository object
type CodeRepositoryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=coderepositories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=coderepositories/status,verbs=get;update;patch

func (r *CodeRepositoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("coderepository", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CodeRepository")
...
func (r *CodeRepositoryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.CodeRepository{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.CodeRepository{}.APIVersion, sagemakerv1alpha1.CodeRepository{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.CodeRepository{}).
