...
// HumanTaskUiReconciler reconciles a HumanTaskUi object
type HumanTaskUiReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=humantaskuis,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=humantaskuis/status,verbs=get;update;patch

func (r *HumanTaskUiReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("humantaskui", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HumanTaskUi")
...
func (r *HumanTaskUiReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.HumanTaskUi{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.HumanTaskUi{}.APIVersion, sagemakerv1alpha1.HumanTaskUi{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.HumanTaskUi{}).
