...
// ModelPackageGroupPolicyReconciler reconciles a ModelPackageGroupPolicy object
type ModelPackageGroupPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=modelpackagegrouppolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=modelpackagegrouppolicies/status,verbs=get;update;patch

func (r *ModelPackageGroupPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("modelpackagegrouppolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ModelPackageGroupPolicy")
...
func (r *ModelPackageGroupPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.ModelPackageGroupPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.ModelPackageGroupPolicy{}.APIVersion, sagemakerv1alpha1.ModelPackageGroupPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.ModelPackageGroupPolicy{}).
