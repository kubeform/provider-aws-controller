...
// FlowDefinitionReconciler reconciles a FlowDefinition object
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=flowdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=flowdefinitions/status,verbs=get;update;patch

func (r *FlowDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("flowdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FlowDefinition")
...
func (r *FlowDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.FlowDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.FlowDefinition{}.APIVersion, sagemakerv1alpha1.FlowDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.FlowDefinition{}).
