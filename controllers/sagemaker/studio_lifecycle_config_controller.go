...
// StudioLifecycleConfigReconciler reconciles a StudioLifecycleConfig object
type StudioLifecycleConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=studiolifecycleconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=studiolifecycleconfigs/status,verbs=get;update;patch

func (r *StudioLifecycleConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("studiolifecycleconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StudioLifecycleConfig")
...
func (r *StudioLifecycleConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.StudioLifecycleConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.StudioLifecycleConfig{}.APIVersion, sagemakerv1alpha1.StudioLifecycleConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.StudioLifecycleConfig{}).
