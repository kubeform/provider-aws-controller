...
// WorkforceReconciler reconciles a Workforce object
type WorkforceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=workforces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sagemaker.aws.kubeform.com,resources=workforces/status,verbs=get;update;patch

func (r *WorkforceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workforce", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Workforce")
...
func (r *WorkforceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sagemakerv1alpha1.Workforce{}); err != nil {
			klog.Error(err, "unable to set up auditor", sagemakerv1alpha1.Workforce{}.APIVersion, sagemakerv1alpha1.Workforce{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sagemakerv1alpha1.Workforce{}).
...
func (r *WorkforceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for Workforce")
			return nil
		}
		...
		resourceList := &sagemakerv1alpha1.WorkforceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Workforce")
			return nil
		}
