...
// DomainMailFromReconciler reconciles a DomainMailFrom object
type DomainMailFromReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=domainmailfroms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=domainmailfroms/status,verbs=get;update;patch

func (r *DomainMailFromReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainmailfrom", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainMailFrom")
...
func (r *DomainMailFromReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sesv1alpha1.DomainMailFrom{}); err != nil {
			klog.Error(err, "unable to set up auditor", sesv1alpha1.DomainMailFrom{}.APIVersion, sesv1alpha1.DomainMailFrom{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sesv1alpha1.DomainMailFrom{}).
