...
// ReceiptRuleSetReconciler reconciles a ReceiptRuleSet object
type ReceiptRuleSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=receiptrulesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=receiptrulesets/status,verbs=get;update;patch

func (r *ReceiptRuleSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("receiptruleset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReceiptRuleSet")
...
func (r *ReceiptRuleSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sesv1alpha1.ReceiptRuleSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", sesv1alpha1.ReceiptRuleSet{}.APIVersion, sesv1alpha1.ReceiptRuleSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sesv1alpha1.ReceiptRuleSet{}).
