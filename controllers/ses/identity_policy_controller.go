...
// IdentityPolicyReconciler reconciles a IdentityPolicy object
type IdentityPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=identitypolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=identitypolicies/status,verbs=get;update;patch

func (r *IdentityPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identitypolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityPolicy")
...
func (r *IdentityPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sesv1alpha1.IdentityPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", sesv1alpha1.IdentityPolicy{}.APIVersion, sesv1alpha1.IdentityPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sesv1alpha1.IdentityPolicy{}).
