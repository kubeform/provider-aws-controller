...
// EmailIdentityReconciler reconciles a EmailIdentity object
type EmailIdentityReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=emailidentities,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=emailidentities/status,verbs=get;update;patch

func (r *EmailIdentityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("emailidentity", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EmailIdentity")
...
func (r *EmailIdentityReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sesv1alpha1.EmailIdentity{}); err != nil {
			klog.Error(err, "unable to set up auditor", sesv1alpha1.EmailIdentity{}.APIVersion, sesv1alpha1.EmailIdentity{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sesv1alpha1.EmailIdentity{}).
