...
// ReceiptFilterReconciler reconciles a ReceiptFilter object
type ReceiptFilterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=receiptfilters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ses.aws.kubeform.com,resources=receiptfilters/status,verbs=get;update;patch

func (r *ReceiptFilterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("receiptfilter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReceiptFilter")
...
func (r *ReceiptFilterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sesv1alpha1.ReceiptFilter{}); err != nil {
			klog.Error(err, "unable to set up auditor", sesv1alpha1.ReceiptFilter{}.APIVersion, sesv1alpha1.ReceiptFilter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sesv1alpha1.ReceiptFilter{}).
