...
// RegistryScanningConfigurationReconciler reconciles a RegistryScanningConfiguration object
type RegistryScanningConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=registryscanningconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=registryscanningconfigurations/status,verbs=get;update;patch

func (r *RegistryScanningConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registryscanningconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegistryScanningConfiguration")
...
func (r *RegistryScanningConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecrv1alpha1.RegistryScanningConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecrv1alpha1.RegistryScanningConfiguration{}.APIVersion, ecrv1alpha1.RegistryScanningConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecrv1alpha1.RegistryScanningConfiguration{}).
