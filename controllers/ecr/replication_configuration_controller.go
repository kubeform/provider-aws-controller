...
// ReplicationConfigurationReconciler reconciles a ReplicationConfiguration object
type ReplicationConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=replicationconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=replicationconfigurations/status,verbs=get;update;patch

func (r *ReplicationConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicationconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicationConfiguration")
...
func (r *ReplicationConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecrv1alpha1.ReplicationConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecrv1alpha1.ReplicationConfiguration{}.APIVersion, ecrv1alpha1.ReplicationConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecrv1alpha1.ReplicationConfiguration{}).
