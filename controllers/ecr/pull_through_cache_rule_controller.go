...
// PullThroughCacheRuleReconciler reconciles a PullThroughCacheRule object
type PullThroughCacheRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=pullthroughcacherules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=pullthroughcacherules/status,verbs=get;update;patch

func (r *PullThroughCacheRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pullthroughcacherule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PullThroughCacheRule")
...
func (r *PullThroughCacheRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecrv1alpha1.PullThroughCacheRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecrv1alpha1.PullThroughCacheRule{}.APIVersion, ecrv1alpha1.PullThroughCacheRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecrv1alpha1.PullThroughCacheRule{}).
