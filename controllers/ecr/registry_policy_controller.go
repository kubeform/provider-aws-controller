...
// RegistryPolicyReconciler reconciles a RegistryPolicy object
type RegistryPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=registrypolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecr.aws.kubeform.com,resources=registrypolicies/status,verbs=get;update;patch

func (r *RegistryPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registrypolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegistryPolicy")
...
func (r *RegistryPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecrv1alpha1.RegistryPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecrv1alpha1.RegistryPolicy{}.APIVersion, ecrv1alpha1.RegistryPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecrv1alpha1.RegistryPolicy{}).
