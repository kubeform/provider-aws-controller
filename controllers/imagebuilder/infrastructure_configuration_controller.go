...
// InfrastructureConfigurationReconciler reconciles a InfrastructureConfiguration object
type InfrastructureConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=infrastructureconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=infrastructureconfigurations/status,verbs=get;update;patch

func (r *InfrastructureConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("infrastructureconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InfrastructureConfiguration")
...
func (r *InfrastructureConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagebuilderv1alpha1.InfrastructureConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagebuilderv1alpha1.InfrastructureConfiguration{}.APIVersion, imagebuilderv1alpha1.InfrastructureConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagebuilderv1alpha1.InfrastructureConfiguration{}).
