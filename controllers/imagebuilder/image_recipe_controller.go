...
// ImageRecipeReconciler reconciles a ImageRecipe object
type ImageRecipeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=imagerecipes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=imagerecipes/status,verbs=get;update;patch

func (r *ImageRecipeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("imagerecipe", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ImageRecipe")
...
func (r *ImageRecipeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagebuilderv1alpha1.ImageRecipe{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagebuilderv1alpha1.ImageRecipe{}.APIVersion, imagebuilderv1alpha1.ImageRecipe{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagebuilderv1alpha1.ImageRecipe{}).
