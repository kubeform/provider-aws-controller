...
// ContainerRecipeReconciler reconciles a ContainerRecipe object
type ContainerRecipeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=containerrecipes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=imagebuilder.aws.kubeform.com,resources=containerrecipes/status,verbs=get;update;patch

func (r *ContainerRecipeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerrecipe", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerRecipe")
...
func (r *ContainerRecipeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &imagebuilderv1alpha1.ContainerRecipe{}); err != nil {
			klog.Error(err, "unable to set up auditor", imagebuilderv1alpha1.ContainerRecipe{}.APIVersion, imagebuilderv1alpha1.ContainerRecipe{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&imagebuilderv1alpha1.ContainerRecipe{}).
