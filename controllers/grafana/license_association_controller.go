...
// LicenseAssociationReconciler reconciles a LicenseAssociation object
type LicenseAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=grafana.aws.kubeform.com,resources=licenseassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=grafana.aws.kubeform.com,resources=licenseassociations/status,verbs=get;update;patch

func (r *LicenseAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("licenseassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LicenseAssociation")
...
func (r *LicenseAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &grafanav1alpha1.LicenseAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", grafanav1alpha1.LicenseAssociation{}.APIVersion, grafanav1alpha1.LicenseAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&grafanav1alpha1.LicenseAssociation{}).
