...
// TopicPolicyReconciler reconciles a TopicPolicy object
type TopicPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=topicpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=topicpolicies/status,verbs=get;update;patch

func (r *TopicPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("topicpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TopicPolicy")
...
func (r *TopicPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &snsv1alpha1.TopicPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", snsv1alpha1.TopicPolicy{}.APIVersion, snsv1alpha1.TopicPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&snsv1alpha1.TopicPolicy{}).
