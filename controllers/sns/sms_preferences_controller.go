...
// SmsPreferencesReconciler reconciles a SmsPreferences object
type SmsPreferencesReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=smspreferences,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=smspreferences/status,verbs=get;update;patch

func (r *SmsPreferencesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("smspreferences", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SmsPreferences")
...
func (r *SmsPreferencesReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &snsv1alpha1.SmsPreferences{}); err != nil {
			klog.Error(err, "unable to set up auditor", snsv1alpha1.SmsPreferences{}.APIVersion, snsv1alpha1.SmsPreferences{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&snsv1alpha1.SmsPreferences{}).
