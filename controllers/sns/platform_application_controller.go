...
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=platformapplications,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sns.aws.kubeform.com,resources=platformapplications/status,verbs=get;update;patch

func (r *PlatformApplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("platformapplication", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PlatformApplication")
...
func (r *PlatformApplicationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &snsv1alpha1.PlatformApplication{}); err != nil {
			klog.Error(err, "unable to set up auditor", snsv1alpha1.PlatformApplication{}.APIVersion, snsv1alpha1.PlatformApplication{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&snsv1alpha1.PlatformApplication{}).
...
func (r *PlatformApplicationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PlatformApplication")
			return nil
		}
		...
		resourceList := &snsv1alpha1.PlatformApplicationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PlatformApplication")
			return nil
		}
