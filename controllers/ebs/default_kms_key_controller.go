...
// DefaultKmsKeyReconciler reconciles a DefaultKmsKey object
type DefaultKmsKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=defaultkmskeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=defaultkmskeys/status,verbs=get;update;patch

func (r *DefaultKmsKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("defaultkmskey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DefaultKmsKey")
...
func (r *DefaultKmsKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ebsv1alpha1.DefaultKmsKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", ebsv1alpha1.DefaultKmsKey{}.APIVersion, ebsv1alpha1.DefaultKmsKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ebsv1alpha1.DefaultKmsKey{}).
