...
// EncryptionByDefaultReconciler reconciles a EncryptionByDefault object
type EncryptionByDefaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=encryptionbydefaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=encryptionbydefaults/status,verbs=get;update;patch

func (r *EncryptionByDefaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("encryptionbydefault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EncryptionByDefault")
...
func (r *EncryptionByDefaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ebsv1alpha1.EncryptionByDefault{}); err != nil {
			klog.Error(err, "unable to set up auditor", ebsv1alpha1.EncryptionByDefault{}.APIVersion, ebsv1alpha1.EncryptionByDefault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ebsv1alpha1.EncryptionByDefault{}).
