...
// SnapshotImportReconciler reconciles a SnapshotImport object
...
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=snapshotimports,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=snapshotimports/status,verbs=get;update;patch

func (r *SnapshotImportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snapshotimport", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnapshotImport")
...
func (r *SnapshotImportReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ebsv1alpha1.SnapshotImport{}); err != nil {
			klog.Error(err, "unable to set up auditor", ebsv1alpha1.SnapshotImport{}.APIVersion, ebsv1alpha1.SnapshotImport{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ebsv1alpha1.SnapshotImport{}).
