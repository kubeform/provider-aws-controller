...
// SnapshotCopyReconciler reconciles a SnapshotCopy object
type SnapshotCopyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=snapshotcopies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ebs.aws.kubeform.com,resources=snapshotcopies/status,verbs=get;update;patch

func (r *SnapshotCopyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snapshotcopy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnapshotCopy")
...
func (r *SnapshotCopyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ebsv1alpha1.SnapshotCopy{}); err != nil {
			klog.Error(err, "unable to set up auditor", ebsv1alpha1.SnapshotCopy{}.APIVersion, ebsv1alpha1.SnapshotCopy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ebsv1alpha1.SnapshotCopy{}).
