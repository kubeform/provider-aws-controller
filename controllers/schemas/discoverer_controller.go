...
// DiscovererReconciler reconciles a Discoverer object
type DiscovererReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=schemas.aws.kubeform.com,resources=discoverers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=schemas.aws.kubeform.com,resources=discoverers/status,verbs=get;update;patch

func (r *DiscovererReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("discoverer", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Discoverer")
...
func (r *DiscovererReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &schemasv1alpha1.Discoverer{}); err != nil {
			klog.Error(err, "unable to set up auditor", schemasv1alpha1.Discoverer{}.APIVersion, schemasv1alpha1.Discoverer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&schemasv1alpha1.Discoverer{}).
