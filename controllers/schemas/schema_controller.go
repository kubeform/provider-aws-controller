...
// +kubebuilder:rbac:groups=schemas.aws.kubeform.com,resources=schemas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=schemas.aws.kubeform.com,resources=schemas/status,verbs=get;update;patch

func (r *SchemaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("schema", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Schema")
...
func (r *SchemaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &schemasv1alpha1.Schema{}); err != nil {
			klog.Error(err, "unable to set up auditor", schemasv1alpha1.Schema{}.APIVersion, schemasv1alpha1.Schema{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&schemasv1alpha1.Schema{}).
