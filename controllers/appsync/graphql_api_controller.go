...
// GraphqlAPIReconciler reconciles a GraphqlAPI object
...
// +kubebuilder:rbac:groups=appsync.aws.kubeform.com,resources=graphqlapis,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appsync.aws.kubeform.com,resources=graphqlapis/status,verbs=get;update;patch

func (r *GraphqlAPIReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("graphqlapi", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GraphqlAPI")
...
func (r *GraphqlAPIReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appsyncv1alpha1.GraphqlAPI{}); err != nil {
			klog.Error(err, "unable to set up auditor", appsyncv1alpha1.GraphqlAPI{}.APIVersion, appsyncv1alpha1.GraphqlAPI{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appsyncv1alpha1.GraphqlAPI{}).
