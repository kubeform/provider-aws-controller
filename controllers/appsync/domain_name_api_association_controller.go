...
// DomainNameAPIAssociationReconciler reconciles a DomainNameAPIAssociation object
type DomainNameAPIAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appsync.aws.kubeform.com,resources=domainnameapiassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appsync.aws.kubeform.com,resources=domainnameapiassociations/status,verbs=get;update;patch

func (r *DomainNameAPIAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainnameapiassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainNameAPIAssociation")
...
func (r *DomainNameAPIAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appsyncv1alpha1.DomainNameAPIAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", appsyncv1alpha1.DomainNameAPIAssociation{}.APIVersion, appsyncv1alpha1.DomainNameAPIAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appsyncv1alpha1.DomainNameAPIAssociation{}).
