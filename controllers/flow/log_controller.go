...
// +kubebuilder:rbac:groups=flow.aws.kubeform.com,resources=logs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=flow.aws.kubeform.com,resources=logs/status,verbs=get;update;patch

func (r *LogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("log", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Log")
...
func (r *LogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &flowv1alpha1.Log{}); err != nil {
			klog.Error(err, "unable to set up auditor", flowv1alpha1.Log{}.APIVersion, flowv1alpha1.Log{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&flowv1alpha1.Log{}).
