...
// RegisteredDomainReconciler reconciles a RegisteredDomain object
type RegisteredDomainReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53domains.aws.kubeform.com,resources=registereddomains,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53domains.aws.kubeform.com,resources=registereddomains/status,verbs=get;update;patch

func (r *RegisteredDomainReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("registereddomain", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegisteredDomain")
...
func (r *RegisteredDomainReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53domainsv1alpha1.RegisteredDomain{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53domainsv1alpha1.RegisteredDomain{}.APIVersion, route53domainsv1alpha1.RegisteredDomain{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53domainsv1alpha1.RegisteredDomain{}).
