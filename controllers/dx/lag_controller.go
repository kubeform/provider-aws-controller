...
// LagReconciler reconciles a Lag object
type LagReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=lags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=lags/status,verbs=get;update;patch

func (r *LagReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lag", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Lag")
...
func (r *LagReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.Lag{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.Lag{}.APIVersion, dxv1alpha1.Lag{}.Kind)
			return err
		}
	}
