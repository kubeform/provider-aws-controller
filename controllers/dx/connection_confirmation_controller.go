...
// ConnectionConfirmationReconciler reconciles a ConnectionConfirmation object
type ConnectionConfirmationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=connectionconfirmations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=connectionconfirmations/status,verbs=get;update;patch

func (r *ConnectionConfirmationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectionconfirmation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionConfirmation")
...
func (r *ConnectionConfirmationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.ConnectionConfirmation{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.ConnectionConfirmation{}.APIVersion, dxv1alpha1.ConnectionConfirmation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dxv1alpha1.ConnectionConfirmation{}).
