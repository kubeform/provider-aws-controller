...
// TransitVirtualInterfaceReconciler reconciles a TransitVirtualInterface object
type TransitVirtualInterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=transitvirtualinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=transitvirtualinterfaces/status,verbs=get;update;patch

func (r *TransitVirtualInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("transitvirtualinterface", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TransitVirtualInterface")
...
func (r *TransitVirtualInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.TransitVirtualInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.TransitVirtualInterface{}.APIVersion, dxv1alpha1.TransitVirtualInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dxv1alpha1.TransitVirtualInterface{}).
