...
// PrivateVirtualInterfaceReconciler reconciles a PrivateVirtualInterface object
type PrivateVirtualInterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=privatevirtualinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=privatevirtualinterfaces/status,verbs=get;update;patch

func (r *PrivateVirtualInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatevirtualinterface", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateVirtualInterface")
...
func (r *PrivateVirtualInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.PrivateVirtualInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.PrivateVirtualInterface{}.APIVersion, dxv1alpha1.PrivateVirtualInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dxv1alpha1.PrivateVirtualInterface{}).
