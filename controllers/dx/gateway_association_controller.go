...
// GatewayAssociationReconciler reconciles a GatewayAssociation object
type GatewayAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=gatewayassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=gatewayassociations/status,verbs=get;update;patch

func (r *GatewayAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewayassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayAssociation")
...
func (r *GatewayAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.GatewayAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.GatewayAssociation{}.APIVersion, dxv1alpha1.GatewayAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dxv1alpha1.GatewayAssociation{}).
