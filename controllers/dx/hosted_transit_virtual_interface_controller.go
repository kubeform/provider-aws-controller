...
// HostedTransitVirtualInterfaceReconciler reconciles a HostedTransitVirtualInterface object
type HostedTransitVirtualInterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=hostedtransitvirtualinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dx.aws.kubeform.com,resources=hostedtransitvirtualinterfaces/status,verbs=get;update;patch

func (r *HostedTransitVirtualInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostedtransitvirtualinterface", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostedTransitVirtualInterface")
...
func (r *HostedTransitVirtualInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dxv1alpha1.HostedTransitVirtualInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", dxv1alpha1.HostedTransitVirtualInterface{}.APIVersion, dxv1alpha1.HostedTransitVirtualInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dxv1alpha1.HostedTransitVirtualInterface{}).
