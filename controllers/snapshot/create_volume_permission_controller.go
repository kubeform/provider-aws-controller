...
// CreateVolumePermissionReconciler reconciles a CreateVolumePermission object
type CreateVolumePermissionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=snapshot.aws.kubeform.com,resources=createvolumepermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=snapshot.aws.kubeform.com,resources=createvolumepermissions/status,verbs=get;update;patch

func (r *CreateVolumePermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("createvolumepermission", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CreateVolumePermission")
...
func (r *CreateVolumePermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &snapshotv1alpha1.CreateVolumePermission{}); err != nil {
			klog.Error(err, "unable to set up auditor", snapshotv1alpha1.CreateVolumePermission{}.APIVersion, snapshotv1alpha1.CreateVolumePermission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&snapshotv1alpha1.CreateVolumePermission{}).
