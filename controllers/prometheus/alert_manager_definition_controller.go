...
// AlertManagerDefinitionReconciler reconciles a AlertManagerDefinition object
type AlertManagerDefinitionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=prometheus.aws.kubeform.com,resources=alertmanagerdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=prometheus.aws.kubeform.com,resources=alertmanagerdefinitions/status,verbs=get;update;patch

func (r *AlertManagerDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertmanagerdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertManagerDefinition")
...
func (r *AlertManagerDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &prometheusv1alpha1.AlertManagerDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", prometheusv1alpha1.AlertManagerDefinition{}.APIVersion, prometheusv1alpha1.AlertManagerDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&prometheusv1alpha1.AlertManagerDefinition{}).
