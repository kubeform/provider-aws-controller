...
// RuleGroupNamespaceReconciler reconciles a RuleGroupNamespace object
type RuleGroupNamespaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=prometheus.aws.kubeform.com,resources=rulegroupnamespaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=prometheus.aws.kubeform.com,resources=rulegroupnamespaces/status,verbs=get;update;patch

func (r *RuleGroupNamespaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("rulegroupnamespace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RuleGroupNamespace")
...
func (r *RuleGroupNamespaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &prometheusv1alpha1.RuleGroupNamespace{}); err != nil {
			klog.Error(err, "unable to set up auditor", prometheusv1alpha1.RuleGroupNamespace{}.APIVersion, prometheusv1alpha1.RuleGroupNamespace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&prometheusv1alpha1.RuleGroupNamespace{}).
