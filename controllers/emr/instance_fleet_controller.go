...
// InstanceFleetReconciler reconciles a InstanceFleet object
type InstanceFleetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=instancefleet,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=instancefleet/status,verbs=get;update;patch

func (r *InstanceFleetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("instancefleet", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstanceFleet")
...
func (r *InstanceFleetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &emrv1alpha1.InstanceFleet{}); err != nil {
			klog.Error(err, "unable to set up auditor", emrv1alpha1.InstanceFleet{}.APIVersion, emrv1alpha1.InstanceFleet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&emrv1alpha1.InstanceFleet{}).
