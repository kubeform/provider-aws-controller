...
// StudioSessionMappingReconciler reconciles a StudioSessionMapping object
type StudioSessionMappingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=studiosessionmappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=studiosessionmappings/status,verbs=get;update;patch

func (r *StudioSessionMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("studiosessionmapping", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StudioSessionMapping")
...
func (r *StudioSessionMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &emrv1alpha1.StudioSessionMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", emrv1alpha1.StudioSessionMapping{}.APIVersion, emrv1alpha1.StudioSessionMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&emrv1alpha1.StudioSessionMapping{}).
