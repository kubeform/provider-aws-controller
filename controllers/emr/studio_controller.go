...
// StudioReconciler reconciles a Studio object
type StudioReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=studios,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=emr.aws.kubeform.com,resources=studios/status,verbs=get;update;patch

func (r *StudioReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("studio", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Studio")
...
func (r *StudioReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &emrv1alpha1.Studio{}); err != nil {
			klog.Error(err, "unable to set up auditor", emrv1alpha1.Studio{}.APIVersion, emrv1alpha1.Studio{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&emrv1alpha1.Studio{}).
