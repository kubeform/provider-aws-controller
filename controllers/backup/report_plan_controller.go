...
// ReportPlanReconciler reconciles a ReportPlan object
type ReportPlanReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=reportplans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=reportplans/status,verbs=get;update;patch

func (r *ReportPlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reportplan", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReportPlan")
...
func (r *ReportPlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.ReportPlan{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.ReportPlan{}.APIVersion, backupv1alpha1.ReportPlan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.ReportPlan{}).
