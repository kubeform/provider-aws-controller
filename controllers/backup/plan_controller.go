...
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=plans,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=plans/status,verbs=get;update;patch
...
func (r *PlanReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Plan")
...
func (r *PlanReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.Plan{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.Plan{}.APIVersion, backupv1alpha1.Plan{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.Plan{}).
