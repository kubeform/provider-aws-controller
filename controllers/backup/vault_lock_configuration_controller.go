...
// VaultLockConfigurationReconciler reconciles a VaultLockConfiguration object
type VaultLockConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=vaultlockconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=vaultlockconfigurations/status,verbs=get;update;patch

func (r *VaultLockConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vaultlockconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VaultLockConfiguration")
...
func (r *VaultLockConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.VaultLockConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.VaultLockConfiguration{}.APIVersion, backupv1alpha1.VaultLockConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.VaultLockConfiguration{}).
