...
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=frameworks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=frameworks/status,verbs=get;update;patch

func (r *FrameworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("framework", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Framework")
...
func (r *FrameworkReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.Framework{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.Framework{}.APIVersion, backupv1alpha1.Framework{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.Framework{}).
