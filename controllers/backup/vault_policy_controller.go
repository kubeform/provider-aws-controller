...
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=vaultpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=backup.aws.kubeform.com,resources=vaultpolicies/status,verbs=get;update;patch

func (r *VaultPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vaultpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VaultPolicy")
...
func (r *VaultPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &backupv1alpha1.VaultPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", backupv1alpha1.VaultPolicy{}.APIVersion, backupv1alpha1.VaultPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&backupv1alpha1.VaultPolicy{}).
