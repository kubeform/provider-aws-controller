...
// TableAssociationReconciler reconciles a TableAssociation object
type TableAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route.aws.kubeform.com,resources=tableassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route.aws.kubeform.com,resources=tableassociations/status,verbs=get;update;patch

func (r *TableAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tableassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TableAssociation")
...
func (r *TableAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &routev1alpha1.TableAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", routev1alpha1.TableAssociation{}.APIVersion, routev1alpha1.TableAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&routev1alpha1.TableAssociation{}).
