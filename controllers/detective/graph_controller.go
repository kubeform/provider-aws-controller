...
// +kubebuilder:rbac:groups=detective.aws.kubeform.com,resources=graphs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=detective.aws.kubeform.com,resources=graphs/status,verbs=get;update;patch

func (r *GraphReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("graph", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Graph")
...
func (r *GraphReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &detectivev1alpha1.Graph{}); err != nil {
			klog.Error(err, "unable to set up auditor", detectivev1alpha1.Graph{}.APIVersion, detectivev1alpha1.Graph{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&detectivev1alpha1.Graph{}).
