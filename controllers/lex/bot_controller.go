...
// +kubebuilder:rbac:groups=lex.aws.kubeform.com,resources=bots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lex.aws.kubeform.com,resources=bots/status,verbs=get;update;patch

func (r *BotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Bot")
...
func (r *BotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lexv1alpha1.Bot{}); err != nil {
			klog.Error(err, "unable to set up auditor", lexv1alpha1.Bot{}.APIVersion, lexv1alpha1.Bot{}.Kind)
			return err
		}
	}
