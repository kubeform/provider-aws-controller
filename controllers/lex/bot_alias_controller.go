...
// BotAliasReconciler reconciles a BotAlias object
type BotAliasReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lex.aws.kubeform.com,resources=botalias,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lex.aws.kubeform.com,resources=botalias/status,verbs=get;update;patch

func (r *BotAliasReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("botalias", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BotAlias")
...
func (r *BotAliasReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lexv1alpha1.BotAlias{}); err != nil {
			klog.Error(err, "unable to set up auditor", lexv1alpha1.BotAlias{}.APIVersion, lexv1alpha1.BotAlias{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lexv1alpha1.BotAlias{}).
