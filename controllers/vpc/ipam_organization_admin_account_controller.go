...
// IpamOrganizationAdminAccountReconciler reconciles a IpamOrganizationAdminAccount object
type IpamOrganizationAdminAccountReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipamorganizationadminaccounts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipamorganizationadminaccounts/status,verbs=get;update;patch

func (r *IpamOrganizationAdminAccountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipamorganizationadminaccount", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpamOrganizationAdminAccount")
...
func (r *IpamOrganizationAdminAccountReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.IpamOrganizationAdminAccount{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.IpamOrganizationAdminAccount{}.APIVersion, vpcv1alpha1.IpamOrganizationAdminAccount{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.IpamOrganizationAdminAccount{}).
