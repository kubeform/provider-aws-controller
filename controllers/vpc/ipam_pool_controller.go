...
// IpamPoolReconciler reconciles a IpamPool object
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampools/status,verbs=get;update;patch

func (r *IpamPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipampool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpamPool")
...
func (r *IpamPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.IpamPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.IpamPool{}.APIVersion, vpcv1alpha1.IpamPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.IpamPool{}).
