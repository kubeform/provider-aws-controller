...
// PeeringConnectionAccepterReconciler reconciles a PeeringConnectionAccepter object
type PeeringConnectionAccepterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=peeringconnectionaccepters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=peeringconnectionaccepters/status,verbs=get;update;patch

func (r *PeeringConnectionAccepterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("peeringconnectionaccepter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PeeringConnectionAccepter")
...
func (r *PeeringConnectionAccepterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.PeeringConnectionAccepter{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.PeeringConnectionAccepter{}.APIVersion, vpcv1alpha1.PeeringConnectionAccepter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.PeeringConnectionAccepter{}).
