...
// IpamPoolCIDRAllocationReconciler reconciles a IpamPoolCIDRAllocation object
type IpamPoolCIDRAllocationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampoolcidrallocations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampoolcidrallocations/status,verbs=get;update;patch

func (r *IpamPoolCIDRAllocationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipampoolcidrallocation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpamPoolCIDRAllocation")
...
func (r *IpamPoolCIDRAllocationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.IpamPoolCIDRAllocation{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.IpamPoolCIDRAllocation{}.APIVersion, vpcv1alpha1.IpamPoolCIDRAllocation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.IpamPoolCIDRAllocation{}).
