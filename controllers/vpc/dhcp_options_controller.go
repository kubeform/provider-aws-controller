...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=dhcpoptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=dhcpoptions/status,verbs=get;update;patch

func (r *DhcpOptionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dhcpoptions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DhcpOptions")
...
func (r *DhcpOptionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.DhcpOptions{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.DhcpOptions{}.APIVersion, vpcv1alpha1.DhcpOptions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.DhcpOptions{}).
