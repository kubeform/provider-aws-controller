...
// PeeringConnectionOptionsReconciler reconciles a PeeringConnectionOptions object
type PeeringConnectionOptionsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=peeringconnectionoptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=peeringconnectionoptions/status,verbs=get;update;patch

func (r *PeeringConnectionOptionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("peeringconnectionoptions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PeeringConnectionOptions")
...
func (r *PeeringConnectionOptionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.PeeringConnectionOptions{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.PeeringConnectionOptions{}.APIVersion, vpcv1alpha1.PeeringConnectionOptions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.PeeringConnectionOptions{}).
