...
// EndpointSubnetAssociationReconciler reconciles a EndpointSubnetAssociation object
type EndpointSubnetAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=endpointsubnetassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=endpointsubnetassociations/status,verbs=get;update;patch

func (r *EndpointSubnetAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointsubnetassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointSubnetAssociation")
...
func (r *EndpointSubnetAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.EndpointSubnetAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.EndpointSubnetAssociation{}.APIVersion, vpcv1alpha1.EndpointSubnetAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.EndpointSubnetAssociation{}).
