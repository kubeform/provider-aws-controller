...
// IpamPoolCIDRReconciler reconciles a IpamPoolCIDR object
type IpamPoolCIDRReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampoolcidrs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipampoolcidrs/status,verbs=get;update;patch

func (r *IpamPoolCIDRReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipampoolcidr", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpamPoolCIDR")
...
func (r *IpamPoolCIDRReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.IpamPoolCIDR{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.IpamPoolCIDR{}.APIVersion, vpcv1alpha1.IpamPoolCIDR{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.IpamPoolCIDR{}).
