...
// Ipv6CIDRBlockAssociationReconciler reconciles a Ipv6CIDRBlockAssociation object
type Ipv6CIDRBlockAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipv6cidrblockassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipv6cidrblockassociations/status,verbs=get;update;patch

func (r *Ipv6CIDRBlockAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipv6cidrblockassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipv6CIDRBlockAssociation")
...
func (r *Ipv6CIDRBlockAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.Ipv6CIDRBlockAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.Ipv6CIDRBlockAssociation{}.APIVersion, vpcv1alpha1.Ipv6CIDRBlockAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.Ipv6CIDRBlockAssociation{}).
