...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipams/status,verbs=get;update;patch
...
func (r *IpamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipam")
...
func (r *IpamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.Ipam{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.Ipam{}.APIVersion, vpcv1alpha1.Ipam{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.Ipam{}).
