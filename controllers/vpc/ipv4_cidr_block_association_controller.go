...
// Ipv4CIDRBlockAssociationReconciler reconciles a Ipv4CIDRBlockAssociation object
type Ipv4CIDRBlockAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipv4cidrblockassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=ipv4cidrblockassociations/status,verbs=get;update;patch

func (r *Ipv4CIDRBlockAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipv4cidrblockassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ipv4CIDRBlockAssociation")
...
func (r *Ipv4CIDRBlockAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.Ipv4CIDRBlockAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.Ipv4CIDRBlockAssociation{}.APIVersion, vpcv1alpha1.Ipv4CIDRBlockAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.Ipv4CIDRBlockAssociation{}).
