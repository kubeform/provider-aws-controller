...
// EndpointPolicyReconciler reconciles a EndpointPolicy object
...
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=endpointpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpc.aws.kubeform.com,resources=endpointpolicies/status,verbs=get;update;patch

func (r *EndpointPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("endpointpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EndpointPolicy")
...
func (r *EndpointPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpcv1alpha1.EndpointPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpcv1alpha1.EndpointPolicy{}.APIVersion, vpcv1alpha1.EndpointPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpcv1alpha1.EndpointPolicy{}).
