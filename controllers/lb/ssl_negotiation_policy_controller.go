...
// SslNegotiationPolicyReconciler reconciles a SslNegotiationPolicy object
type SslNegotiationPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lb.aws.kubeform.com,resources=sslnegotiationpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lb.aws.kubeform.com,resources=sslnegotiationpolicies/status,verbs=get;update;patch

func (r *SslNegotiationPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sslnegotiationpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SslNegotiationPolicy")
...
func (r *SslNegotiationPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lbv1alpha1.SslNegotiationPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", lbv1alpha1.SslNegotiationPolicy{}.APIVersion, lbv1alpha1.SslNegotiationPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lbv1alpha1.SslNegotiationPolicy{}).
