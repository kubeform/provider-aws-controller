...
// +kubebuilder:rbac:groups=worklink.aws.kubeform.com,resources=fleet,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=worklink.aws.kubeform.com,resources=fleet/status,verbs=get;update;patch

func (r *FleetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fleet", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Fleet")
...
func (r *FleetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &worklinkv1alpha1.Fleet{}); err != nil {
			klog.Error(err, "unable to set up auditor", worklinkv1alpha1.Fleet{}.APIVersion, worklinkv1alpha1.Fleet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&worklinkv1alpha1.Fleet{}).
