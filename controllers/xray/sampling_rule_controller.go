...
// SamplingRuleReconciler reconciles a SamplingRule object
...
// +kubebuilder:rbac:groups=xray.aws.kubeform.com,resources=samplingrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=xray.aws.kubeform.com,resources=samplingrules/status,verbs=get;update;patch

func (r *SamplingRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("samplingrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SamplingRule")
...
func (r *SamplingRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &xrayv1alpha1.SamplingRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", xrayv1alpha1.SamplingRule{}.APIVersion, xrayv1alpha1.SamplingRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&xrayv1alpha1.SamplingRule{}).
