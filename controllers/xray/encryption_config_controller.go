...
// EncryptionConfigReconciler reconciles a EncryptionConfig object
type EncryptionConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=xray.aws.kubeform.com,resources=encryptionconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=xray.aws.kubeform.com,resources=encryptionconfigs/status,verbs=get;update;patch

func (r *EncryptionConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("encryptionconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EncryptionConfig")
...
func (r *EncryptionConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &xrayv1alpha1.EncryptionConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", xrayv1alpha1.EncryptionConfig{}.APIVersion, xrayv1alpha1.EncryptionConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&xrayv1alpha1.EncryptionConfig{}).
