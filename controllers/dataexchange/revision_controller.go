...
// +kubebuilder:rbac:groups=dataexchange.aws.kubeform.com,resources=revisions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dataexchange.aws.kubeform.com,resources=revisions/status,verbs=get;update;patch
...
func (r *RevisionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Revision")
...
func (r *RevisionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dataexchangev1alpha1.Revision{}); err != nil {
			klog.Error(err, "unable to set up auditor", dataexchangev1alpha1.Revision{}.APIVersion, dataexchangev1alpha1.Revision{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dataexchangev1alpha1.Revision{}).
