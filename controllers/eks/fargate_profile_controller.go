...
// +kubebuilder:rbac:groups=eks.aws.kubeform.com,resources=fargateprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eks.aws.kubeform.com,resources=fargateprofiles/status,verbs=get;update;patch

func (r *FargateProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("fargateprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FargateProfile")
...
func (r *FargateProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eksv1alpha1.FargateProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", eksv1alpha1.FargateProfile{}.APIVersion, eksv1alpha1.FargateProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eksv1alpha1.FargateProfile{}).
