...
// IdentityProviderConfigReconciler reconciles a IdentityProviderConfig object
type IdentityProviderConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=eks.aws.kubeform.com,resources=identityproviderconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=eks.aws.kubeform.com,resources=identityproviderconfigs/status,verbs=get;update;patch

func (r *IdentityProviderConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identityproviderconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityProviderConfig")
...
func (r *IdentityProviderConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eksv1alpha1.IdentityProviderConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", eksv1alpha1.IdentityProviderConfig{}.APIVersion, eksv1alpha1.IdentityProviderConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eksv1alpha1.IdentityProviderConfig{}).
