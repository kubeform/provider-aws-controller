...
// LaunchPermissionReconciler reconciles a LaunchPermission object
type LaunchPermissionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=launchpermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=launchpermissions/status,verbs=get;update;patch

func (r *LaunchPermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("launchpermission", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LaunchPermission")
...
func (r *LaunchPermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amiv1alpha1.LaunchPermission{}); err != nil {
			klog.Error(err, "unable to set up auditor", amiv1alpha1.LaunchPermission{}.APIVersion, amiv1alpha1.LaunchPermission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amiv1alpha1.LaunchPermission{}).
