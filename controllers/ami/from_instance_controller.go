...
// FromInstanceReconciler reconciles a FromInstance object
type FromInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=frominstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=frominstances/status,verbs=get;update;patch

func (r *FromInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("frominstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FromInstance")
...
func (r *FromInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amiv1alpha1.FromInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", amiv1alpha1.FromInstance{}.APIVersion, amiv1alpha1.FromInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&amiv1alpha1.FromInstance{}).
