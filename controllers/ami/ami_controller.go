...
// AmiReconciler reconciles a Ami object
...
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=amis,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ami.aws.kubeform.com,resources=amis/status,verbs=get;update;patch

func (r *AmiReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ami", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ami")
...
func (r *AmiReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &amiv1alpha1.Ami{}); err != nil {
			klog.Error(err, "unable to set up auditor", amiv1alpha1.Ami{}.APIVersion, amiv1alpha1.Ami{}.Kind)
			return err
		}
	}
