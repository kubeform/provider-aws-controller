...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=layerversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=layerversions/status,verbs=get;update;patch

func (r *LayerVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LayerVersion")
...
func (r *LayerVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.LayerVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.LayerVersion{}.APIVersion, lambdav1alpha1.LayerVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.LayerVersion{}).
