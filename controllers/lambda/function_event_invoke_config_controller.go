...
// FunctionEventInvokeConfigReconciler reconciles a FunctionEventInvokeConfig object
type FunctionEventInvokeConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=functioneventinvokeconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=functioneventinvokeconfigs/status,verbs=get;update;patch

func (r *FunctionEventInvokeConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("functioneventinvokeconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FunctionEventInvokeConfig")
...
func (r *FunctionEventInvokeConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.FunctionEventInvokeConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.FunctionEventInvokeConfig{}.APIVersion, lambdav1alpha1.FunctionEventInvokeConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.FunctionEventInvokeConfig{}).
