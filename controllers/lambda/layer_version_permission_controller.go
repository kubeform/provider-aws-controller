...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=layerversionpermissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=layerversionpermissions/status,verbs=get;update;patch

func (r *LayerVersionPermissionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LayerVersionPermission")
...
func (r *LayerVersionPermissionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.LayerVersionPermission{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.LayerVersionPermission{}.APIVersion, lambdav1alpha1.LayerVersionPermission{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.LayerVersionPermission{}).
