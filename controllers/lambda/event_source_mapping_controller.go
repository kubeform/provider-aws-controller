...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=eventsourcemappings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=eventsourcemappings/status,verbs=get;update;patch

func (r *EventSourceMappingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventSourceMapping")
...
func (r *EventSourceMappingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.EventSourceMapping{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.EventSourceMapping{}.APIVersion, lambdav1alpha1.EventSourceMapping{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.EventSourceMapping{}).
