...
// InvocationReconciler reconciles a Invocation object
type InvocationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=invocations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=invocations/status,verbs=get;update;patch

func (r *InvocationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("invocation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Invocation")
...
func (r *InvocationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.Invocation{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.Invocation{}.APIVersion, lambdav1alpha1.Invocation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.Invocation{}).
