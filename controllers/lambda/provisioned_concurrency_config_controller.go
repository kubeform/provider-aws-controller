...
// ProvisionedConcurrencyConfigReconciler reconciles a ProvisionedConcurrencyConfig object
type ProvisionedConcurrencyConfigReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=provisionedconcurrencyconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lambda.aws.kubeform.com,resources=provisionedconcurrencyconfigs/status,verbs=get;update;patch

func (r *ProvisionedConcurrencyConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("provisionedconcurrencyconfig", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProvisionedConcurrencyConfig")
...
func (r *ProvisionedConcurrencyConfigReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lambdav1alpha1.ProvisionedConcurrencyConfig{}); err != nil {
			klog.Error(err, "unable to set up auditor", lambdav1alpha1.ProvisionedConcurrencyConfig{}.APIVersion, lambdav1alpha1.ProvisionedConcurrencyConfig{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lambdav1alpha1.ProvisionedConcurrencyConfig{}).
