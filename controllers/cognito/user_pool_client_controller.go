...
// UserPoolClientReconciler reconciles a UserPoolClient object
...
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpoolclients,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpoolclients/status,verbs=get;update;patch

func (r *UserPoolClientReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userpoolclient", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserPoolClient")
...
func (r *UserPoolClientReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitov1alpha1.UserPoolClient{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitov1alpha1.UserPoolClient{}.APIVersion, cognitov1alpha1.UserPoolClient{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitov1alpha1.UserPoolClient{}).
...
func (r *UserPoolClientReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for UserPoolClient")
			return nil
		}
		...
		resourceList := &cognitov1alpha1.UserPoolClientList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list UserPoolClient")
			return nil
		}
