...
// UserPoolReconciler reconciles a UserPool object
...
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpools/status,verbs=get;update;patch

func (r *UserPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userpool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserPool")
...
func (r *UserPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitov1alpha1.UserPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitov1alpha1.UserPool{}.APIVersion, cognitov1alpha1.UserPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitov1alpha1.UserPool{}).
