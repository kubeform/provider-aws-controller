...
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=identityproviders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=identityproviders/status,verbs=get;update;patch

func (r *IdentityProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identityprovider", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityProvider")
...
func (r *IdentityProviderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitov1alpha1.IdentityProvider{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitov1alpha1.IdentityProvider{}.APIVersion, cognitov1alpha1.IdentityProvider{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitov1alpha1.IdentityProvider{}).
