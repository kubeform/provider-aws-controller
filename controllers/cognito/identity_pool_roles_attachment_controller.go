...
// IdentityPoolRolesAttachmentReconciler reconciles a IdentityPoolRolesAttachment object
type IdentityPoolRolesAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=identitypoolrolesattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=identitypoolrolesattachments/status,verbs=get;update;patch

func (r *IdentityPoolRolesAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identitypoolrolesattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityPoolRolesAttachment")
...
func (r *IdentityPoolRolesAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitov1alpha1.IdentityPoolRolesAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitov1alpha1.IdentityPoolRolesAttachment{}.APIVersion, cognitov1alpha1.IdentityPoolRolesAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitov1alpha1.IdentityPoolRolesAttachment{}).
