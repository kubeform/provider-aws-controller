...
// UserPoolUiCustomizationReconciler reconciles a UserPoolUiCustomization object
type UserPoolUiCustomizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpooluicustomizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cognito.aws.kubeform.com,resources=userpooluicustomizations/status,verbs=get;update;patch

func (r *UserPoolUiCustomizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userpooluicustomization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserPoolUiCustomization")
...
func (r *UserPoolUiCustomizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cognitov1alpha1.UserPoolUiCustomization{}); err != nil {
			klog.Error(err, "unable to set up auditor", cognitov1alpha1.UserPoolUiCustomization{}.APIVersion, cognitov1alpha1.UserPoolUiCustomization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cognitov1alpha1.UserPoolUiCustomization{}).
