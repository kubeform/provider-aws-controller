...
// StaticIPReconciler reconciles a StaticIP object
type StaticIPReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=lightsail.aws.kubeform.com,resources=staticips,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=lightsail.aws.kubeform.com,resources=staticips/status,verbs=get;update;patch

func (r *StaticIPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("staticip", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StaticIP")
...
func (r *StaticIPReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &lightsailv1alpha1.StaticIP{}); err != nil {
			klog.Error(err, "unable to set up auditor", lightsailv1alpha1.StaticIP{}.APIVersion, lightsailv1alpha1.StaticIP{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&lightsailv1alpha1.StaticIP{}).
