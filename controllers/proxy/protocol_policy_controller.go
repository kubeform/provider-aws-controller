...
// ProtocolPolicyReconciler reconciles a ProtocolPolicy object
type ProtocolPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=proxy.aws.kubeform.com,resources=protocolpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=proxy.aws.kubeform.com,resources=protocolpolicies/status,verbs=get;update;patch

func (r *ProtocolPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("protocolpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProtocolPolicy")
...
func (r *ProtocolPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &proxyv1alpha1.ProtocolPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", proxyv1alpha1.ProtocolPolicy{}.APIVersion, proxyv1alpha1.ProtocolPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&proxyv1alpha1.ProtocolPolicy{}).
