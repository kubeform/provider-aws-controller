...
// RoutingControlReconciler reconciles a RoutingControl object
type RoutingControlReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=routingcontrols,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=routingcontrols/status,verbs=get;update;patch

func (r *RoutingControlReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routingcontrol", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RoutingControl")
...
func (r *RoutingControlReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53recoverycontrolconfigv1alpha1.RoutingControl{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53recoverycontrolconfigv1alpha1.RoutingControl{}.APIVersion, route53recoverycontrolconfigv1alpha1.RoutingControl{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53recoverycontrolconfigv1alpha1.RoutingControl{}).
