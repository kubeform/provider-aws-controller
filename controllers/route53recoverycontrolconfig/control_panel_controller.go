...
// ControlPanelReconciler reconciles a ControlPanel object
type ControlPanelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=controlpanels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=controlpanels/status,verbs=get;update;patch

func (r *ControlPanelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("controlpanel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ControlPanel")
...
func (r *ControlPanelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53recoverycontrolconfigv1alpha1.ControlPanel{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53recoverycontrolconfigv1alpha1.ControlPanel{}.APIVersion, route53recoverycontrolconfigv1alpha1.ControlPanel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53recoverycontrolconfigv1alpha1.ControlPanel{}).
