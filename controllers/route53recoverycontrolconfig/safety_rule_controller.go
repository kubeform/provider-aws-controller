...
// SafetyRuleReconciler reconciles a SafetyRule object
type SafetyRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=safetyrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53recoverycontrolconfig.aws.kubeform.com,resources=safetyrules/status,verbs=get;update;patch

func (r *SafetyRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("safetyrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SafetyRule")
...
func (r *SafetyRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53recoverycontrolconfigv1alpha1.SafetyRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53recoverycontrolconfigv1alpha1.SafetyRule{}.APIVersion, route53recoverycontrolconfigv1alpha1.SafetyRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53recoverycontrolconfigv1alpha1.SafetyRule{}).
