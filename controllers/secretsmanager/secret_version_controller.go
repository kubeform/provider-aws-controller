...
// +kubebuilder:rbac:groups=secretsmanager.aws.kubeform.com,resources=secretversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secretsmanager.aws.kubeform.com,resources=secretversions/status,verbs=get;update;patch

func (r *SecretVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("secretversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecretVersion")
...
func (r *SecretVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretsmanagerv1alpha1.SecretVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretsmanagerv1alpha1.SecretVersion{}.APIVersion, secretsmanagerv1alpha1.SecretVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretsmanagerv1alpha1.SecretVersion{}).
...
func (r *SecretVersionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SecretVersion")
			return nil
		}
		...
		resourceList := &secretsmanagerv1alpha1.SecretVersionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SecretVersion")
			return nil
		}
