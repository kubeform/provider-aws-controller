...
// +kubebuilder:rbac:groups=secretsmanager.aws.kubeform.com,resources=secretpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=secretsmanager.aws.kubeform.com,resources=secretpolicies/status,verbs=get;update;patch
...
func (r *SecretPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecretPolicy")
...
func (r *SecretPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &secretsmanagerv1alpha1.SecretPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", secretsmanagerv1alpha1.SecretPolicy{}.APIVersion, secretsmanagerv1alpha1.SecretPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&secretsmanagerv1alpha1.SecretPolicy{}).
