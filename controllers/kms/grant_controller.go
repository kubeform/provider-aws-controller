...
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=grants,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=grants/status,verbs=get;update;patch
...
func (r *GrantReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Grant")
...
func (r *GrantReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.Grant{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.Grant{}.APIVersion, kmsv1alpha1.Grant{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.Grant{}).
