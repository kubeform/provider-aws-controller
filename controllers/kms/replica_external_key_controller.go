...
// ReplicaExternalKeyReconciler reconciles a ReplicaExternalKey object
type ReplicaExternalKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=replicaexternalkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=replicaexternalkeys/status,verbs=get;update;patch

func (r *ReplicaExternalKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicaexternalkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicaExternalKey")
...
func (r *ReplicaExternalKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.ReplicaExternalKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.ReplicaExternalKey{}.APIVersion, kmsv1alpha1.ReplicaExternalKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.ReplicaExternalKey{}).
...
func (r *ReplicaExternalKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ReplicaExternalKey")
			return nil
		}
		...
		resourceList := &kmsv1alpha1.ReplicaExternalKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ReplicaExternalKey")
			return nil
		}
