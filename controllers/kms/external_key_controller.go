...
// ExternalKeyReconciler reconciles a ExternalKey object
type ExternalKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=externalkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=externalkeys/status,verbs=get;update;patch

func (r *ExternalKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("externalkey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ExternalKey")
...
func (r *ExternalKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.ExternalKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.ExternalKey{}.APIVersion, kmsv1alpha1.ExternalKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.ExternalKey{}).
...
func (r *ExternalKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ExternalKey")
			return nil
		}
		...
		resourceList := &kmsv1alpha1.ExternalKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ExternalKey")
			return nil
		}
