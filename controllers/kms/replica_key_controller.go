...
// ReplicaKeyReconciler reconciles a ReplicaKey object
type ReplicaKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=replicakeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kms.aws.kubeform.com,resources=replicakeys/status,verbs=get;update;patch

func (r *ReplicaKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicakey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicaKey")
...
func (r *ReplicaKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &kmsv1alpha1.ReplicaKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", kmsv1alpha1.ReplicaKey{}.APIVersion, kmsv1alpha1.ReplicaKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&kmsv1alpha1.ReplicaKey{}).
