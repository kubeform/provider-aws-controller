...
// +kubebuilder:rbac:groups=sfn.aws.kubeform.com,resources=statemachines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sfn.aws.kubeform.com,resources=statemachines/status,verbs=get;update;patch

func (r *StateMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("statemachine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StateMachine")
...
func (r *StateMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sfnv1alpha1.StateMachine{}); err != nil {
			klog.Error(err, "unable to set up auditor", sfnv1alpha1.StateMachine{}.APIVersion, sfnv1alpha1.StateMachine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sfnv1alpha1.StateMachine{}).
