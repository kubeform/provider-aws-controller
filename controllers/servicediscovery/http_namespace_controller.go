...
// HttpNamespaceReconciler reconciles a HttpNamespace object
type HttpNamespaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicediscovery.aws.kubeform.com,resources=httpnamespaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicediscovery.aws.kubeform.com,resources=httpnamespaces/status,verbs=get;update;patch

func (r *HttpNamespaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("httpnamespace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HttpNamespace")
...
func (r *HttpNamespaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicediscoveryv1alpha1.HttpNamespace{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicediscoveryv1alpha1.HttpNamespace{}.APIVersion, servicediscoveryv1alpha1.HttpNamespace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicediscoveryv1alpha1.HttpNamespace{}).
