...
// PrivateDNSNamespaceReconciler reconciles a PrivateDNSNamespace object
type PrivateDNSNamespaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=servicediscovery.aws.kubeform.com,resources=privatednsnamespaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicediscovery.aws.kubeform.com,resources=privatednsnamespaces/status,verbs=get;update;patch

func (r *PrivateDNSNamespaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatednsnamespace", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateDNSNamespace")
...
func (r *PrivateDNSNamespaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicediscoveryv1alpha1.PrivateDNSNamespace{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicediscoveryv1alpha1.PrivateDNSNamespace{}.APIVersion, servicediscoveryv1alpha1.PrivateDNSNamespace{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicediscoveryv1alpha1.PrivateDNSNamespace{}).
