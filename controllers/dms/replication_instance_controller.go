...
// ReplicationInstanceReconciler reconciles a ReplicationInstance object
type ReplicationInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dms.aws.kubeform.com,resources=replicationinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dms.aws.kubeform.com,resources=replicationinstances/status,verbs=get;update;patch

func (r *ReplicationInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replicationinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReplicationInstance")
...
func (r *ReplicationInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dmsv1alpha1.ReplicationInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", dmsv1alpha1.ReplicationInstance{}.APIVersion, dmsv1alpha1.ReplicationInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dmsv1alpha1.ReplicationInstance{}).
