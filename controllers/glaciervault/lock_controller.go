...
// +kubebuilder:rbac:groups=glaciervault.aws.kubeform.com,resources=locks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glaciervault.aws.kubeform.com,resources=locks/status,verbs=get;update;patch

func (r *LockReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lock", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Lock")
...
func (r *LockReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &glaciervaultv1alpha1.Lock{}); err != nil {
			klog.Error(err, "unable to set up auditor", glaciervaultv1alpha1.Lock{}.APIVersion, glaciervaultv1alpha1.Lock{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&glaciervaultv1alpha1.Lock{}).
