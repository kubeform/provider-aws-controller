...
// GlacierVaultReconciler reconciles a GlacierVault object
type GlacierVaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=glaciervault.aws.kubeform.com,resources=glaciervaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glaciervault.aws.kubeform.com,resources=glaciervaults/status,verbs=get;update;patch

func (r *GlacierVaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("glaciervault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlacierVault")
...
func (r *GlacierVaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &glaciervaultv1alpha1.GlacierVault{}); err != nil {
			klog.Error(err, "unable to set up auditor", glaciervaultv1alpha1.GlacierVault{}.APIVersion, glaciervaultv1alpha1.GlacierVault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&glaciervaultv1alpha1.GlacierVault{}).
