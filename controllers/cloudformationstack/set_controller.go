...
// +kubebuilder:rbac:groups=cloudformationstack.aws.kubeform.com,resources=sets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudformationstack.aws.kubeform.com,resources=sets/status,verbs=get;update;patch

func (r *SetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("set", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Set")
...
func (r *SetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudformationstackv1alpha1.Set{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudformationstackv1alpha1.Set{}.APIVersion, cloudformationstackv1alpha1.Set{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudformationstackv1alpha1.Set{}).
