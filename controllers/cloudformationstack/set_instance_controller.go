...
// SetInstanceReconciler reconciles a SetInstance object
type SetInstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudformationstack.aws.kubeform.com,resources=setinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudformationstack.aws.kubeform.com,resources=setinstances/status,verbs=get;update;patch

func (r *SetInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("setinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SetInstance")
...
func (r *SetInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudformationstackv1alpha1.SetInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudformationstackv1alpha1.SetInstance{}.APIVersion, cloudformationstackv1alpha1.SetInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudformationstackv1alpha1.SetInstance{}).
