...
// ReportGroupReconciler reconciles a ReportGroup object
type ReportGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=codebuild.aws.kubeform.com,resources=reportgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=codebuild.aws.kubeform.com,resources=reportgroups/status,verbs=get;update;patch

func (r *ReportGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reportgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReportGroup")
...
func (r *ReportGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &codebuildv1alpha1.ReportGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", codebuildv1alpha1.ReportGroup{}.APIVersion, codebuildv1alpha1.ReportGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&codebuildv1alpha1.ReportGroup{}).
