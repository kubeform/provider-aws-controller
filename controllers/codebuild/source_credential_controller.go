...
// SourceCredentialReconciler reconciles a SourceCredential object
type SourceCredentialReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=codebuild.aws.kubeform.com,resources=sourcecredentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=codebuild.aws.kubeform.com,resources=sourcecredentials/status,verbs=get;update;patch

func (r *SourceCredentialReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sourcecredential", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SourceCredential")
...
func (r *SourceCredentialReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &codebuildv1alpha1.SourceCredential{}); err != nil {
			klog.Error(err, "unable to set up auditor", codebuildv1alpha1.SourceCredential{}.APIVersion, codebuildv1alpha1.SourceCredential{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&codebuildv1alpha1.SourceCredential{}).
...
func (r *SourceCredentialReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SourceCredential")
			return nil
		}
		...
		resourceList := &codebuildv1alpha1.SourceCredentialList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SourceCredential")
			return nil
		}
