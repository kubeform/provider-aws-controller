...
// SubnetGroupReconciler reconciles a SubnetGroup object
type SubnetGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=subnetgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=subnetgroups/status,verbs=get;update;patch

func (r *SubnetGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("subnetgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SubnetGroup")
...
func (r *SubnetGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &neptunev1alpha1.SubnetGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", neptunev1alpha1.SubnetGroup{}.APIVersion, neptunev1alpha1.SubnetGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&neptunev1alpha1.SubnetGroup{}).
