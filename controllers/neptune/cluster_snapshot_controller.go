...
// ClusterSnapshotReconciler reconciles a ClusterSnapshot object
type ClusterSnapshotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clustersnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clustersnapshots/status,verbs=get;update;patch

func (r *ClusterSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustersnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterSnapshot")
...
func (r *ClusterSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &neptunev1alpha1.ClusterSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", neptunev1alpha1.ClusterSnapshot{}.APIVersion, neptunev1alpha1.ClusterSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&neptunev1alpha1.ClusterSnapshot{}).
