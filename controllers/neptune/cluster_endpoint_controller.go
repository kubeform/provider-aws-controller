...
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clusterendpoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clusterendpoints/status,verbs=get;update;patch

func (r *ClusterEndpointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterendpoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterEndpoint")
...
func (r *ClusterEndpointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &neptunev1alpha1.ClusterEndpoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", neptunev1alpha1.ClusterEndpoint{}.APIVersion, neptunev1alpha1.ClusterEndpoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&neptunev1alpha1.ClusterEndpoint{}).
