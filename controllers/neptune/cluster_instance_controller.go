...
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clusterinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=clusterinstances/status,verbs=get;update;patch

func (r *ClusterInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterinstance", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterInstance")
...
func (r *ClusterInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &neptunev1alpha1.ClusterInstance{}); err != nil {
			klog.Error(err, "unable to set up auditor", neptunev1alpha1.ClusterInstance{}.APIVersion, neptunev1alpha1.ClusterInstance{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&neptunev1alpha1.ClusterInstance{}).
