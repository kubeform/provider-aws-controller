...
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=eventsubscriptions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=neptune.aws.kubeform.com,resources=eventsubscriptions/status,verbs=get;update;patch
...
func (r *EventSubscriptionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventSubscription")
...
func (r *EventSubscriptionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &neptunev1alpha1.EventSubscription{}); err != nil {
			klog.Error(err, "unable to set up auditor", neptunev1alpha1.EventSubscription{}.APIVersion, neptunev1alpha1.EventSubscription{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&neptunev1alpha1.EventSubscription{}).
