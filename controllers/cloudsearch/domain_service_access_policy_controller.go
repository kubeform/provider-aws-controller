...
// DomainServiceAccessPolicyReconciler reconciles a DomainServiceAccessPolicy object
type DomainServiceAccessPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudsearch.aws.kubeform.com,resources=domainserviceaccesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudsearch.aws.kubeform.com,resources=domainserviceaccesspolicies/status,verbs=get;update;patch

func (r *DomainServiceAccessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("domainserviceaccesspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DomainServiceAccessPolicy")
...
func (r *DomainServiceAccessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudsearchv1alpha1.DomainServiceAccessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudsearchv1alpha1.DomainServiceAccessPolicy{}.APIVersion, cloudsearchv1alpha1.DomainServiceAccessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudsearchv1alpha1.DomainServiceAccessPolicy{}).
