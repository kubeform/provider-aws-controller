...
// +kubebuilder:rbac:groups=appmesh.aws.kubeform.com,resources=virtualgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appmesh.aws.kubeform.com,resources=virtualgateways/status,verbs=get;update;patch

func (r *VirtualGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualGateway")
...
func (r *VirtualGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appmeshv1alpha1.VirtualGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", appmeshv1alpha1.VirtualGateway{}.APIVersion, appmeshv1alpha1.VirtualGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appmeshv1alpha1.VirtualGateway{}).
