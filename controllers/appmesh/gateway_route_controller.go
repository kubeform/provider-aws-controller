...
// +kubebuilder:rbac:groups=appmesh.aws.kubeform.com,resources=gatewayroutes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appmesh.aws.kubeform.com,resources=gatewayroutes/status,verbs=get;update;patch

func (r *GatewayRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewayroute", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayRoute")
...
func (r *GatewayRouteReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appmeshv1alpha1.GatewayRoute{}); err != nil {
			klog.Error(err, "unable to set up auditor", appmeshv1alpha1.GatewayRoute{}.APIVersion, appmeshv1alpha1.GatewayRoute{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appmeshv1alpha1.GatewayRoute{}).
