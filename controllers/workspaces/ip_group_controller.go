...
// IpGroupReconciler reconciles a IpGroup object
type IpGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=workspaces.aws.kubeform.com,resources=ipgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=workspaces.aws.kubeform.com,resources=ipgroups/status,verbs=get;update;patch

func (r *IpGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpGroup")
...
func (r *IpGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &workspacesv1alpha1.IpGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", workspacesv1alpha1.IpGroup{}.APIVersion, workspacesv1alpha1.IpGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&workspacesv1alpha1.IpGroup{}).
