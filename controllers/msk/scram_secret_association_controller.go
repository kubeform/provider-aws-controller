...
// ScramSecretAssociationReconciler reconciles a ScramSecretAssociation object
type ScramSecretAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=msk.aws.kubeform.com,resources=scramsecretassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=msk.aws.kubeform.com,resources=scramsecretassociations/status,verbs=get;update;patch

func (r *ScramSecretAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("scramsecretassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ScramSecretAssociation")
...
func (r *ScramSecretAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mskv1alpha1.ScramSecretAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", mskv1alpha1.ScramSecretAssociation{}.APIVersion, mskv1alpha1.ScramSecretAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mskv1alpha1.ScramSecretAssociation{}).
