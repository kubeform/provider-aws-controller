...
// +kubebuilder:rbac:groups=sqsqueue.aws.kubeform.com,resources=sqsqueues,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=sqsqueue.aws.kubeform.com,resources=sqsqueues/status,verbs=get;update;patch

func (r *SqsQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("sqsqueue", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SqsQueue")
...
func (r *SqsQueueReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &sqsqueuev1alpha1.SqsQueue{}); err != nil {
			klog.Error(err, "unable to set up auditor", sqsqueuev1alpha1.SqsQueue{}.APIVersion, sqsqueuev1alpha1.SqsQueue{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&sqsqueuev1alpha1.SqsQueue{}).
