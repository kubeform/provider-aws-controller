...
// OpenzfsFileSystemReconciler reconciles a OpenzfsFileSystem object
type OpenzfsFileSystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=openzfsfilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=openzfsfilesystems/status,verbs=get;update;patch

func (r *OpenzfsFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("openzfsfilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OpenzfsFileSystem")
...
func (r *OpenzfsFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fsxv1alpha1.OpenzfsFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", fsxv1alpha1.OpenzfsFileSystem{}.APIVersion, fsxv1alpha1.OpenzfsFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fsxv1alpha1.OpenzfsFileSystem{}).
