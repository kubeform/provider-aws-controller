...
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=lustrefilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=lustrefilesystems/status,verbs=get;update;patch

func (r *LustreFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("lustrefilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LustreFileSystem")
...
func (r *LustreFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fsxv1alpha1.LustreFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", fsxv1alpha1.LustreFileSystem{}.APIVersion, fsxv1alpha1.LustreFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fsxv1alpha1.LustreFileSystem{}).
