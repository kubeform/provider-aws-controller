...
// WindowsFileSystemReconciler reconciles a WindowsFileSystem object
type WindowsFileSystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=windowsfilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=windowsfilesystems/status,verbs=get;update;patch

func (r *WindowsFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("windowsfilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WindowsFileSystem")
...
func (r *WindowsFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fsxv1alpha1.WindowsFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", fsxv1alpha1.WindowsFileSystem{}.APIVersion, fsxv1alpha1.WindowsFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fsxv1alpha1.WindowsFileSystem{}).
...
func (r *WindowsFileSystemReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for WindowsFileSystem")
			return nil
		}
		...
		resourceList := &fsxv1alpha1.WindowsFileSystemList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list WindowsFileSystem")
			return nil
		}
