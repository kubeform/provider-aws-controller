...
// OpenzfsSnapshotReconciler reconciles a OpenzfsSnapshot object
type OpenzfsSnapshotReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=openzfssnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=openzfssnapshots/status,verbs=get;update;patch

func (r *OpenzfsSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("openzfssnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OpenzfsSnapshot")
...
func (r *OpenzfsSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fsxv1alpha1.OpenzfsSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", fsxv1alpha1.OpenzfsSnapshot{}.APIVersion, fsxv1alpha1.OpenzfsSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fsxv1alpha1.OpenzfsSnapshot{}).
