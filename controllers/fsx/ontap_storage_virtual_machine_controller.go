...
// OntapStorageVirtualMachineReconciler reconciles a OntapStorageVirtualMachine object
type OntapStorageVirtualMachineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=ontapstoragevirtualmachines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.aws.kubeform.com,resources=ontapstoragevirtualmachines/status,verbs=get;update;patch

func (r *OntapStorageVirtualMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ontapstoragevirtualmachine", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OntapStorageVirtualMachine")
...
func (r *OntapStorageVirtualMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &fsxv1alpha1.OntapStorageVirtualMachine{}); err != nil {
			klog.Error(err, "unable to set up auditor", fsxv1alpha1.OntapStorageVirtualMachine{}.APIVersion, fsxv1alpha1.OntapStorageVirtualMachine{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&fsxv1alpha1.OntapStorageVirtualMachine{}).
...
func (r *OntapStorageVirtualMachineReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for OntapStorageVirtualMachine")
			return nil
		}
		...
		resourceList := &fsxv1alpha1.OntapStorageVirtualMachineList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list OntapStorageVirtualMachine")
			return nil
		}
