...
// WebACLReconciler reconciles a WebACL object
...
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=webacls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=webacls/status,verbs=get;update;patch

func (r *WebACLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("webacl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WebACL")
...
func (r *WebACLReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafregionalv1alpha1.WebACL{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafregionalv1alpha1.WebACL{}.APIVersion, wafregionalv1alpha1.WebACL{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafregionalv1alpha1.WebACL{}).
