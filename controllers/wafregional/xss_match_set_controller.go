...
// XssMatchSetReconciler reconciles a XssMatchSet object
type XssMatchSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=xssmatchsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=xssmatchsets/status,verbs=get;update;patch

func (r *XssMatchSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("xssmatchset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch XssMatchSet")
...
func (r *XssMatchSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafregionalv1alpha1.XssMatchSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafregionalv1alpha1.XssMatchSet{}.APIVersion, wafregionalv1alpha1.XssMatchSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafregionalv1alpha1.XssMatchSet{}).
