...
// RegexMatchSetReconciler reconciles a RegexMatchSet object
type RegexMatchSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=regexmatchsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=regexmatchsets/status,verbs=get;update;patch

func (r *RegexMatchSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("regexmatchset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegexMatchSet")
...
func (r *RegexMatchSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafregionalv1alpha1.RegexMatchSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafregionalv1alpha1.RegexMatchSet{}.APIVersion, wafregionalv1alpha1.RegexMatchSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafregionalv1alpha1.RegexMatchSet{}).
