...
// GeoMatchSetReconciler reconciles a GeoMatchSet object
type GeoMatchSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=geomatchsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=geomatchsets/status,verbs=get;update;patch

func (r *GeoMatchSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("geomatchset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GeoMatchSet")
...
func (r *GeoMatchSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafregionalv1alpha1.GeoMatchSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafregionalv1alpha1.GeoMatchSet{}.APIVersion, wafregionalv1alpha1.GeoMatchSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafregionalv1alpha1.GeoMatchSet{}).
