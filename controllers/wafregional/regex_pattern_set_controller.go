...
// RegexPatternSetReconciler reconciles a RegexPatternSet object
type RegexPatternSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=regexpatternsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=wafregional.aws.kubeform.com,resources=regexpatternsets/status,verbs=get;update;patch

func (r *RegexPatternSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("regexpatternset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RegexPatternSet")
...
func (r *RegexPatternSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &wafregionalv1alpha1.RegexPatternSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", wafregionalv1alpha1.RegexPatternSet{}.APIVersion, wafregionalv1alpha1.RegexPatternSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&wafregionalv1alpha1.RegexPatternSet{}).
