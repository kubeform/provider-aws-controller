...
// GlobalTableReconciler reconciles a GlobalTable object
type GlobalTableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=globaltables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=globaltables/status,verbs=get;update;patch

func (r *GlobalTableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globaltable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalTable")
...
func (r *GlobalTableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dynamodbv1alpha1.GlobalTable{}); err != nil {
			klog.Error(err, "unable to set up auditor", dynamodbv1alpha1.GlobalTable{}.APIVersion, dynamodbv1alpha1.GlobalTable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dynamodbv1alpha1.GlobalTable{}).
