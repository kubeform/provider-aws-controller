...
// KinesisStreamingDestinationReconciler reconciles a KinesisStreamingDestination object
type KinesisStreamingDestinationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=kinesisstreamingdestinations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=kinesisstreamingdestinations/status,verbs=get;update;patch

func (r *KinesisStreamingDestinationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("kinesisstreamingdestination", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch KinesisStreamingDestination")
...
func (r *KinesisStreamingDestinationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dynamodbv1alpha1.KinesisStreamingDestination{}); err != nil {
			klog.Error(err, "unable to set up auditor", dynamodbv1alpha1.KinesisStreamingDestination{}.APIVersion, dynamodbv1alpha1.KinesisStreamingDestination{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dynamodbv1alpha1.KinesisStreamingDestination{}).
