...
// TableItemReconciler reconciles a TableItem object
type TableItemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=tableitems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dynamodb.aws.kubeform.com,resources=tableitems/status,verbs=get;update;patch

func (r *TableItemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tableitem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TableItem")
...
func (r *TableItemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dynamodbv1alpha1.TableItem{}); err != nil {
			klog.Error(err, "unable to set up auditor", dynamodbv1alpha1.TableItem{}.APIVersion, dynamodbv1alpha1.TableItem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dynamodbv1alpha1.TableItem{}).
