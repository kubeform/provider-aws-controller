...
// ConnectionRouteReconciler reconciles a ConnectionRoute object
type ConnectionRouteReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpn.aws.kubeform.com,resources=connectionroutes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpn.aws.kubeform.com,resources=connectionroutes/status,verbs=get;update;patch

func (r *ConnectionRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("connectionroute", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionRoute")
...
func (r *ConnectionRouteReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpnv1alpha1.ConnectionRoute{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpnv1alpha1.ConnectionRoute{}.APIVersion, vpnv1alpha1.ConnectionRoute{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpnv1alpha1.ConnectionRoute{}).
