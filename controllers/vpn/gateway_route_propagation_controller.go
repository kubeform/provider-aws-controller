...
// GatewayRoutePropagationReconciler reconciles a GatewayRoutePropagation object
type GatewayRoutePropagationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vpn.aws.kubeform.com,resources=gatewayroutepropagations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vpn.aws.kubeform.com,resources=gatewayroutepropagations/status,verbs=get;update;patch

func (r *GatewayRoutePropagationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gatewayroutepropagation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GatewayRoutePropagation")
...
func (r *GatewayRoutePropagationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vpnv1alpha1.GatewayRoutePropagation{}); err != nil {
			klog.Error(err, "unable to set up auditor", vpnv1alpha1.GatewayRoutePropagation{}.APIVersion, vpnv1alpha1.GatewayRoutePropagation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vpnv1alpha1.GatewayRoutePropagation{}).
