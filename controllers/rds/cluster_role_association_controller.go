...
// ClusterRoleAssociationReconciler reconciles a ClusterRoleAssociation object
type ClusterRoleAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=rds.aws.kubeform.com,resources=clusterroleassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=rds.aws.kubeform.com,resources=clusterroleassociations/status,verbs=get;update;patch

func (r *ClusterRoleAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clusterroleassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterRoleAssociation")
...
func (r *ClusterRoleAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rdsv1alpha1.ClusterRoleAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", rdsv1alpha1.ClusterRoleAssociation{}.APIVersion, rdsv1alpha1.ClusterRoleAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rdsv1alpha1.ClusterRoleAssociation{}).
