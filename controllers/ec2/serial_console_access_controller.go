...
// SerialConsoleAccessReconciler reconciles a SerialConsoleAccess object
type SerialConsoleAccessReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=serialconsoleaccesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=serialconsoleaccesses/status,verbs=get;update;patch

func (r *SerialConsoleAccessReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serialconsoleaccess", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SerialConsoleAccess")
...
func (r *SerialConsoleAccessReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ec2v1alpha1.SerialConsoleAccess{}); err != nil {
			klog.Error(err, "unable to set up auditor", ec2v1alpha1.SerialConsoleAccess{}.APIVersion, ec2v1alpha1.SerialConsoleAccess{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ec2v1alpha1.SerialConsoleAccess{}).
