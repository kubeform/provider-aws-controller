...
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=capacityreservations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=capacityreservations/status,verbs=get;update;patch

func (r *CapacityReservationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("capacityreservation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CapacityReservation")
...
func (r *CapacityReservationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ec2v1alpha1.CapacityReservation{}); err != nil {
			klog.Error(err, "unable to set up auditor", ec2v1alpha1.CapacityReservation{}.APIVersion, ec2v1alpha1.CapacityReservation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ec2v1alpha1.CapacityReservation{}).
