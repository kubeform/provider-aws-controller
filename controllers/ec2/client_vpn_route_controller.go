...
// ClientVPNRouteReconciler reconciles a ClientVPNRoute object
type ClientVPNRouteReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=clientvpnroutes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=clientvpnroutes/status,verbs=get;update;patch

func (r *ClientVPNRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clientvpnroute", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClientVPNRoute")
...
func (r *ClientVPNRouteReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ec2v1alpha1.ClientVPNRoute{}); err != nil {
			klog.Error(err, "unable to set up auditor", ec2v1alpha1.ClientVPNRoute{}.APIVersion, ec2v1alpha1.ClientVPNRoute{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ec2v1alpha1.ClientVPNRoute{}).
