...
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=transitgateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ec2.aws.kubeform.com,resources=transitgateways/status,verbs=get;update;patch

func (r *TransitGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("transitgateway", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TransitGateway")
...
func (r *TransitGatewayReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ec2v1alpha1.TransitGateway{}); err != nil {
			klog.Error(err, "unable to set up auditor", ec2v1alpha1.TransitGateway{}.APIVersion, ec2v1alpha1.TransitGateway{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ec2v1alpha1.TransitGateway{}).
