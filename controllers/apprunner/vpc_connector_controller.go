...
// VpcConnectorReconciler reconciles a VpcConnector object
type VpcConnectorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apprunner.aws.kubeform.com,resources=vpcconnectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apprunner.aws.kubeform.com,resources=vpcconnectors/status,verbs=get;update;patch

func (r *VpcConnectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vpcconnector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VpcConnector")
...
func (r *VpcConnectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apprunnerv1alpha1.VpcConnector{}); err != nil {
			klog.Error(err, "unable to set up auditor", apprunnerv1alpha1.VpcConnector{}.APIVersion, apprunnerv1alpha1.VpcConnector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apprunnerv1alpha1.VpcConnector{}).
