...
// CustomDomainAssociationReconciler reconciles a CustomDomainAssociation object
type CustomDomainAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apprunner.aws.kubeform.com,resources=customdomainassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apprunner.aws.kubeform.com,resources=customdomainassociations/status,verbs=get;update;patch

func (r *CustomDomainAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customdomainassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomDomainAssociation")
...
func (r *CustomDomainAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apprunnerv1alpha1.CustomDomainAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", apprunnerv1alpha1.CustomDomainAssociation{}.APIVersion, apprunnerv1alpha1.CustomDomainAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apprunnerv1alpha1.CustomDomainAssociation{}).
