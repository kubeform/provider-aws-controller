...
// LocationEfsReconciler reconciles a LocationEfs object
type LocationEfsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationefs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationefs/status,verbs=get;update;patch

func (r *LocationEfsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("locationefs", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LocationEfs")
...
func (r *LocationEfsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasyncv1alpha1.LocationEfs{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasyncv1alpha1.LocationEfs{}.APIVersion, datasyncv1alpha1.LocationEfs{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasyncv1alpha1.LocationEfs{}).
