...
// LocationNfsReconciler reconciles a LocationNfs object
type LocationNfsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationnfs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationnfs/status,verbs=get;update;patch

func (r *LocationNfsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("locationnfs", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LocationNfs")
...
func (r *LocationNfsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasyncv1alpha1.LocationNfs{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasyncv1alpha1.LocationNfs{}.APIVersion, datasyncv1alpha1.LocationNfs{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasyncv1alpha1.LocationNfs{}).
