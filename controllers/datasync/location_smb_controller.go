...
// LocationSmbReconciler reconciles a LocationSmb object
type LocationSmbReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationsmbs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationsmbs/status,verbs=get;update;patch

func (r *LocationSmbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("locationsmb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LocationSmb")
...
func (r *LocationSmbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasyncv1alpha1.LocationSmb{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasyncv1alpha1.LocationSmb{}.APIVersion, datasyncv1alpha1.LocationSmb{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasyncv1alpha1.LocationSmb{}).
...
func (r *LocationSmbReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for LocationSmb")
			return nil
		}
		...
		resourceList := &datasyncv1alpha1.LocationSmbList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list LocationSmb")
			return nil
		}
