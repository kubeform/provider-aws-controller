...
// LocationFsxLustreFileSystemReconciler reconciles a LocationFsxLustreFileSystem object
type LocationFsxLustreFileSystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationfsxlustrefilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationfsxlustrefilesystems/status,verbs=get;update;patch

func (r *LocationFsxLustreFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("locationfsxlustrefilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LocationFsxLustreFileSystem")
...
func (r *LocationFsxLustreFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasyncv1alpha1.LocationFsxLustreFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasyncv1alpha1.LocationFsxLustreFileSystem{}.APIVersion, datasyncv1alpha1.LocationFsxLustreFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasyncv1alpha1.LocationFsxLustreFileSystem{}).
