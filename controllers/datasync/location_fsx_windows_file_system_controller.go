...
// LocationFsxWindowsFileSystemReconciler reconciles a LocationFsxWindowsFileSystem object
type LocationFsxWindowsFileSystemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationfsxwindowsfilesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datasync.aws.kubeform.com,resources=locationfsxwindowsfilesystems/status,verbs=get;update;patch

func (r *LocationFsxWindowsFileSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("locationfsxwindowsfilesystem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LocationFsxWindowsFileSystem")
...
func (r *LocationFsxWindowsFileSystemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datasyncv1alpha1.LocationFsxWindowsFileSystem{}); err != nil {
			klog.Error(err, "unable to set up auditor", datasyncv1alpha1.LocationFsxWindowsFileSystem{}.APIVersion, datasyncv1alpha1.LocationFsxWindowsFileSystem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datasyncv1alpha1.LocationFsxWindowsFileSystem{}).
...
func (r *LocationFsxWindowsFileSystemReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for LocationFsxWindowsFileSystem")
			return nil
		}
		...
		resourceList := &datasyncv1alpha1.LocationFsxWindowsFileSystemList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list LocationFsxWindowsFileSystem")
			return nil
		}
