...
// +kubebuilder:rbac:groups=networkfirewall.aws.kubeform.com,resources=firewallpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networkfirewall.aws.kubeform.com,resources=firewallpolicies/status,verbs=get;update;patch

func (r *FirewallPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("firewallpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch FirewallPolicy")
...
func (r *FirewallPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkfirewallv1alpha1.FirewallPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkfirewallv1alpha1.FirewallPolicy{}.APIVersion, networkfirewallv1alpha1.FirewallPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkfirewallv1alpha1.FirewallPolicy{}).
