...
// +kubebuilder:rbac:groups=networkfirewall.aws.kubeform.com,resources=resourcepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networkfirewall.aws.kubeform.com,resources=resourcepolicies/status,verbs=get;update;patch

func (r *ResourcePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourcepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourcePolicy")
...
func (r *ResourcePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkfirewallv1alpha1.ResourcePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkfirewallv1alpha1.ResourcePolicy{}.APIVersion, networkfirewallv1alpha1.ResourcePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkfirewallv1alpha1.ResourcePolicy{}).
