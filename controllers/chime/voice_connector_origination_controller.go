...
// VoiceConnectorOriginationReconciler reconciles a VoiceConnectorOrigination object
type VoiceConnectorOriginationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectororiginations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectororiginations/status,verbs=get;update;patch

func (r *VoiceConnectorOriginationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("voiceconnectororigination", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VoiceConnectorOrigination")
...
func (r *VoiceConnectorOriginationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &chimev1alpha1.VoiceConnectorOrigination{}); err != nil {
			klog.Error(err, "unable to set up auditor", chimev1alpha1.VoiceConnectorOrigination{}.APIVersion, chimev1alpha1.VoiceConnectorOrigination{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&chimev1alpha1.VoiceConnectorOrigination{}).
