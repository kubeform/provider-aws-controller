...
// VoiceConnectorReconciler reconciles a VoiceConnector object
type VoiceConnectorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectors,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectors/status,verbs=get;update;patch

func (r *VoiceConnectorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("voiceconnector", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VoiceConnector")
...
func (r *VoiceConnectorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &chimev1alpha1.VoiceConnector{}); err != nil {
			klog.Error(err, "unable to set up auditor", chimev1alpha1.VoiceConnector{}.APIVersion, chimev1alpha1.VoiceConnector{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&chimev1alpha1.VoiceConnector{}).
