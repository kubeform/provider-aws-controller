...
// VoiceConnectorStreamingReconciler reconciles a VoiceConnectorStreaming object
type VoiceConnectorStreamingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorstreamings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorstreamings/status,verbs=get;update;patch

func (r *VoiceConnectorStreamingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("voiceconnectorstreaming", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VoiceConnectorStreaming")
...
func (r *VoiceConnectorStreamingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &chimev1alpha1.VoiceConnectorStreaming{}); err != nil {
			klog.Error(err, "unable to set up auditor", chimev1alpha1.VoiceConnectorStreaming{}.APIVersion, chimev1alpha1.VoiceConnectorStreaming{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&chimev1alpha1.VoiceConnectorStreaming{}).
