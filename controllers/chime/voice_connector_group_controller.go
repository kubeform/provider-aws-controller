...
// VoiceConnectorGroupReconciler reconciles a VoiceConnectorGroup object
type VoiceConnectorGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorgroups/status,verbs=get;update;patch

func (r *VoiceConnectorGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("voiceconnectorgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VoiceConnectorGroup")
...
func (r *VoiceConnectorGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &chimev1alpha1.VoiceConnectorGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", chimev1alpha1.VoiceConnectorGroup{}.APIVersion, chimev1alpha1.VoiceConnectorGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&chimev1alpha1.VoiceConnectorGroup{}).
