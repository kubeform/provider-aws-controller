...
// VoiceConnectorTerminationReconciler reconciles a VoiceConnectorTermination object
type VoiceConnectorTerminationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorterminations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=chime.aws.kubeform.com,resources=voiceconnectorterminations/status,verbs=get;update;patch

func (r *VoiceConnectorTerminationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("voiceconnectortermination", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VoiceConnectorTermination")
...
func (r *VoiceConnectorTerminationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &chimev1alpha1.VoiceConnectorTermination{}); err != nil {
			klog.Error(err, "unable to set up auditor", chimev1alpha1.VoiceConnectorTermination{}.APIVersion, chimev1alpha1.VoiceConnectorTermination{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&chimev1alpha1.VoiceConnectorTermination{}).
