...
// +kubebuilder:rbac:groups=organizations.aws.kubeform.com,resources=policyattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=organizations.aws.kubeform.com,resources=policyattachments/status,verbs=get;update;patch
...
func (r *PolicyAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyAttachment")
...
func (r *PolicyAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &organizationsv1alpha1.PolicyAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", organizationsv1alpha1.PolicyAttachment{}.APIVersion, organizationsv1alpha1.PolicyAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&organizationsv1alpha1.PolicyAttachment{}).
