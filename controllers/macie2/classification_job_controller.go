...
// ClassificationJobReconciler reconciles a ClassificationJob object
type ClassificationJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=macie2.aws.kubeform.com,resources=classificationjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=macie2.aws.kubeform.com,resources=classificationjobs/status,verbs=get;update;patch

func (r *ClassificationJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("classificationjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClassificationJob")
...
func (r *ClassificationJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &macie2v1alpha1.ClassificationJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", macie2v1alpha1.ClassificationJob{}.APIVersion, macie2v1alpha1.ClassificationJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&macie2v1alpha1.ClassificationJob{}).
