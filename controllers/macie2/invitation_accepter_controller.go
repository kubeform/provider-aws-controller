...
// InvitationAccepterReconciler reconciles a InvitationAccepter object
type InvitationAccepterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=macie2.aws.kubeform.com,resources=invitationaccepters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=macie2.aws.kubeform.com,resources=invitationaccepters/status,verbs=get;update;patch

func (r *InvitationAccepterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("invitationaccepter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InvitationAccepter")
...
func (r *InvitationAccepterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &macie2v1alpha1.InvitationAccepter{}); err != nil {
			klog.Error(err, "unable to set up auditor", macie2v1alpha1.InvitationAccepter{}.APIVersion, macie2v1alpha1.InvitationAccepter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&macie2v1alpha1.InvitationAccepter{}).
