...
// SigningProfileReconciler reconciles a SigningProfile object
type SigningProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=signer.aws.kubeform.com,resources=signingprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=signer.aws.kubeform.com,resources=signingprofiles/status,verbs=get;update;patch

func (r *SigningProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("signingprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SigningProfile")
...
func (r *SigningProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &signerv1alpha1.SigningProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", signerv1alpha1.SigningProfile{}.APIVersion, signerv1alpha1.SigningProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&signerv1alpha1.SigningProfile{}).
