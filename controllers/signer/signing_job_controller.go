...
// SigningJobReconciler reconciles a SigningJob object
type SigningJobReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=signer.aws.kubeform.com,resources=signingjobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=signer.aws.kubeform.com,resources=signingjobs/status,verbs=get;update;patch

func (r *SigningJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("signingjob", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SigningJob")
...
func (r *SigningJobReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &signerv1alpha1.SigningJob{}); err != nil {
			klog.Error(err, "unable to set up auditor", signerv1alpha1.SigningJob{}.APIVersion, signerv1alpha1.SigningJob{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&signerv1alpha1.SigningJob{}).
