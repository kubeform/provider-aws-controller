...
// ResourceShareReconciler reconciles a ResourceShare object
...
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=resourceshares,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=resourceshares/status,verbs=get;update;patch

func (r *ResourceShareReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourceshare", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceShare")
...
func (r *ResourceShareReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.ResourceShare{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.ResourceShare{}.APIVersion, ramv1alpha1.ResourceShare{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.ResourceShare{}).
