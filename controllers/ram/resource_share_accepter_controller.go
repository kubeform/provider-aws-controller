...
// ResourceShareAccepterReconciler reconciles a ResourceShareAccepter object
type ResourceShareAccepterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=resourceshareaccepters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=resourceshareaccepters/status,verbs=get;update;patch

func (r *ResourceShareAccepterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourceshareaccepter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceShareAccepter")
...
func (r *ResourceShareAccepterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.ResourceShareAccepter{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.ResourceShareAccepter{}.APIVersion, ramv1alpha1.ResourceShareAccepter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.ResourceShareAccepter{}).
