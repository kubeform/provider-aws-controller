...
// PrincipalAssociationReconciler reconciles a PrincipalAssociation object
type PrincipalAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=principalassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ram.aws.kubeform.com,resources=principalassociations/status,verbs=get;update;patch

func (r *PrincipalAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("principalassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrincipalAssociation")
...
func (r *PrincipalAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ramv1alpha1.PrincipalAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", ramv1alpha1.PrincipalAssociation{}.APIVersion, ramv1alpha1.PrincipalAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ramv1alpha1.PrincipalAssociation{}).
