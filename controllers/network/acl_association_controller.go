...
// AclAssociationReconciler reconciles a AclAssociation object
type AclAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=network.aws.kubeform.com,resources=aclassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=network.aws.kubeform.com,resources=aclassociations/status,verbs=get;update;patch

func (r *AclAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aclassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AclAssociation")
...
func (r *AclAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &networkv1alpha1.AclAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", networkv1alpha1.AclAssociation{}.APIVersion, networkv1alpha1.AclAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&networkv1alpha1.AclAssociation{}).
