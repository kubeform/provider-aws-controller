...
// GroupTagReconciler reconciles a GroupTag object
type GroupTagReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=autoscaling.aws.kubeform.com,resources=grouptags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=autoscaling.aws.kubeform.com,resources=grouptags/status,verbs=get;update;patch

func (r *GroupTagReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("grouptag", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GroupTag")
...
func (r *GroupTagReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &autoscalingv1alpha1.GroupTag{}); err != nil {
			klog.Error(err, "unable to set up auditor", autoscalingv1alpha1.GroupTag{}.APIVersion, autoscalingv1alpha1.GroupTag{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&autoscalingv1alpha1.GroupTag{}).
