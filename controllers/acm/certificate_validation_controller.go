...
// CertificateValidationReconciler reconciles a CertificateValidation object
...
// +kubebuilder:rbac:groups=acm.aws.kubeform.com,resources=certificatevalidations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=acm.aws.kubeform.com,resources=certificatevalidations/status,verbs=get;update;patch

func (r *CertificateValidationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificatevalidation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateValidation")
...
func (r *CertificateValidationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &acmv1alpha1.CertificateValidation{}); err != nil {
			klog.Error(err, "unable to set up auditor", acmv1alpha1.CertificateValidation{}.APIVersion, acmv1alpha1.CertificateValidation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&acmv1alpha1.CertificateValidation{}).
