...
// S3BucketAssociationReconciler reconciles a S3BucketAssociation object
type S3BucketAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=macie.aws.kubeform.com,resources=s3bucketassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=macie.aws.kubeform.com,resources=s3bucketassociations/status,verbs=get;update;patch

func (r *S3BucketAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("s3bucketassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch S3BucketAssociation")
...
func (r *S3BucketAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &maciev1alpha1.S3BucketAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", maciev1alpha1.S3BucketAssociation{}.APIVersion, maciev1alpha1.S3BucketAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&maciev1alpha1.S3BucketAssociation{}).
