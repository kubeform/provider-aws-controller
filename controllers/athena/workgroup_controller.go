...
// WorkgroupReconciler reconciles a Workgroup object
type WorkgroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=athena.aws.kubeform.com,resources=workgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=athena.aws.kubeform.com,resources=workgroups/status,verbs=get;update;patch

func (r *WorkgroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Workgroup")
...
func (r *WorkgroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &athenav1alpha1.Workgroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", athenav1alpha1.Workgroup{}.APIVersion, athenav1alpha1.Workgroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&athenav1alpha1.Workgroup{}).
