...
// NamedQueryReconciler reconciles a NamedQuery object
type NamedQueryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=athena.aws.kubeform.com,resources=namedqueries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=athena.aws.kubeform.com,resources=namedqueries/status,verbs=get;update;patch

func (r *NamedQueryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("namedquery", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NamedQuery")
...
func (r *NamedQueryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &athenav1alpha1.NamedQuery{}); err != nil {
			klog.Error(err, "unable to set up auditor", athenav1alpha1.NamedQuery{}.APIVersion, athenav1alpha1.NamedQuery{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&athenav1alpha1.NamedQuery{}).
