...
// GameSessionQueueReconciler reconciles a GameSessionQueue object
type GameSessionQueueReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=gamelift.aws.kubeform.com,resources=gamesessionqueues,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=gamelift.aws.kubeform.com,resources=gamesessionqueues/status,verbs=get;update;patch

func (r *GameSessionQueueReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("gamesessionqueue", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GameSessionQueue")
...
func (r *GameSessionQueueReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &gameliftv1alpha1.GameSessionQueue{}); err != nil {
			klog.Error(err, "unable to set up auditor", gameliftv1alpha1.GameSessionQueue{}.APIVersion, gameliftv1alpha1.GameSessionQueue{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&gameliftv1alpha1.GameSessionQueue{}).
