...
// ServiceQuotaReconciler reconciles a ServiceQuota object
...
// +kubebuilder:rbac:groups=servicequotas.aws.kubeform.com,resources=servicequota,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servicequotas.aws.kubeform.com,resources=servicequota/status,verbs=get;update;patch

func (r *ServiceQuotaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicequota", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceQuota")
...
func (r *ServiceQuotaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicequotasv1alpha1.ServiceQuota{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicequotasv1alpha1.ServiceQuota{}.APIVersion, servicequotasv1alpha1.ServiceQuota{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicequotasv1alpha1.ServiceQuota{}).
