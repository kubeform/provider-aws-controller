...
// CloudtrailReconciler reconciles a Cloudtrail object
type CloudtrailReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudtrail.aws.kubeform.com,resources=cloudtrails,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudtrail.aws.kubeform.com,resources=cloudtrails/status,verbs=get;update;patch

func (r *CloudtrailReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudtrail", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Cloudtrail")
...
func (r *CloudtrailReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudtrailv1alpha1.Cloudtrail{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudtrailv1alpha1.Cloudtrail{}.APIVersion, cloudtrailv1alpha1.Cloudtrail{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudtrailv1alpha1.Cloudtrail{}).
