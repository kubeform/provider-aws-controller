...
// CloudcontrolapiResourceReconciler reconciles a CloudcontrolapiResource object
type CloudcontrolapiResourceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloudcontrolapi.aws.kubeform.com,resources=cloudcontrolapiresources,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloudcontrolapi.aws.kubeform.com,resources=cloudcontrolapiresources/status,verbs=get;update;patch

func (r *CloudcontrolapiResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudcontrolapiresource", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudcontrolapiResource")
...
func (r *CloudcontrolapiResourceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudcontrolapiv1alpha1.CloudcontrolapiResource{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudcontrolapiv1alpha1.CloudcontrolapiResource{}.APIVersion, cloudcontrolapiv1alpha1.CloudcontrolapiResource{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudcontrolapiv1alpha1.CloudcontrolapiResource{}).
...
func (r *CloudcontrolapiResourceReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for CloudcontrolapiResource")
			return nil
		}
		...
		resourceList := &cloudcontrolapiv1alpha1.CloudcontrolapiResourceList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CloudcontrolapiResource")
			return nil
		}
