...
// MultiRegionAccessPointReconciler reconciles a MultiRegionAccessPoint object
type MultiRegionAccessPointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=multiregionaccesspoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=multiregionaccesspoints/status,verbs=get;update;patch

func (r *MultiRegionAccessPointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("multiregionaccesspoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MultiRegionAccessPoint")
...
func (r *MultiRegionAccessPointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3controlv1alpha1.MultiRegionAccessPoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3controlv1alpha1.MultiRegionAccessPoint{}.APIVersion, s3controlv1alpha1.MultiRegionAccessPoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3controlv1alpha1.MultiRegionAccessPoint{}).
