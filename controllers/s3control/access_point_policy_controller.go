...
// AccessPointPolicyReconciler reconciles a AccessPointPolicy object
type AccessPointPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=accesspointpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=accesspointpolicies/status,verbs=get;update;patch

func (r *AccessPointPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accesspointpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccessPointPolicy")
...
func (r *AccessPointPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3controlv1alpha1.AccessPointPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3controlv1alpha1.AccessPointPolicy{}.APIVersion, s3controlv1alpha1.AccessPointPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3controlv1alpha1.AccessPointPolicy{}).
