...
// MultiRegionAccessPointPolicyReconciler reconciles a MultiRegionAccessPointPolicy object
type MultiRegionAccessPointPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=multiregionaccesspointpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=multiregionaccesspointpolicies/status,verbs=get;update;patch

func (r *MultiRegionAccessPointPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("multiregionaccesspointpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MultiRegionAccessPointPolicy")
...
func (r *MultiRegionAccessPointPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3controlv1alpha1.MultiRegionAccessPointPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3controlv1alpha1.MultiRegionAccessPointPolicy{}.APIVersion, s3controlv1alpha1.MultiRegionAccessPointPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3controlv1alpha1.MultiRegionAccessPointPolicy{}).
