...
// ObjectLambdaAccessPointReconciler reconciles a ObjectLambdaAccessPoint object
type ObjectLambdaAccessPointReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=objectlambdaaccesspoints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=objectlambdaaccesspoints/status,verbs=get;update;patch

func (r *ObjectLambdaAccessPointReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("objectlambdaaccesspoint", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ObjectLambdaAccessPoint")
...
func (r *ObjectLambdaAccessPointReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3controlv1alpha1.ObjectLambdaAccessPoint{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3controlv1alpha1.ObjectLambdaAccessPoint{}.APIVersion, s3controlv1alpha1.ObjectLambdaAccessPoint{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3controlv1alpha1.ObjectLambdaAccessPoint{}).
