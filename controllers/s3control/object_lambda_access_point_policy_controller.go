...
// ObjectLambdaAccessPointPolicyReconciler reconciles a ObjectLambdaAccessPointPolicy object
type ObjectLambdaAccessPointPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=objectlambdaaccesspointpolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=s3control.aws.kubeform.com,resources=objectlambdaaccesspointpolicies/status,verbs=get;update;patch

func (r *ObjectLambdaAccessPointPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("objectlambdaaccesspointpolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ObjectLambdaAccessPointPolicy")
...
func (r *ObjectLambdaAccessPointPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &s3controlv1alpha1.ObjectLambdaAccessPointPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", s3controlv1alpha1.ObjectLambdaAccessPointPolicy{}.APIVersion, s3controlv1alpha1.ObjectLambdaAccessPointPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&s3controlv1alpha1.ObjectLambdaAccessPointPolicy{}).
