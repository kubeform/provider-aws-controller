...
// WorkerConfigurationReconciler reconciles a WorkerConfiguration object
type WorkerConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mskconnect.aws.kubeform.com,resources=workerconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mskconnect.aws.kubeform.com,resources=workerconfigurations/status,verbs=get;update;patch

func (r *WorkerConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("workerconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch WorkerConfiguration")
...
func (r *WorkerConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mskconnectv1alpha1.WorkerConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", mskconnectv1alpha1.WorkerConfiguration{}.APIVersion, mskconnectv1alpha1.WorkerConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mskconnectv1alpha1.WorkerConfiguration{}).
