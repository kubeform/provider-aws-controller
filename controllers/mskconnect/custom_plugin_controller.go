...
// CustomPluginReconciler reconciles a CustomPlugin object
type CustomPluginReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=mskconnect.aws.kubeform.com,resources=customplugins,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mskconnect.aws.kubeform.com,resources=customplugins/status,verbs=get;update;patch

func (r *CustomPluginReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customplugin", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomPlugin")
...
func (r *CustomPluginReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mskconnectv1alpha1.CustomPlugin{}); err != nil {
			klog.Error(err, "unable to set up auditor", mskconnectv1alpha1.CustomPlugin{}.APIVersion, mskconnectv1alpha1.CustomPlugin{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mskconnectv1alpha1.CustomPlugin{}).
