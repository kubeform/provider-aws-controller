...
// SnapshotScheduleAssociationReconciler reconciles a SnapshotScheduleAssociation object
type SnapshotScheduleAssociationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=redshift.aws.kubeform.com,resources=snapshotscheduleassociations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redshift.aws.kubeform.com,resources=snapshotscheduleassociations/status,verbs=get;update;patch

func (r *SnapshotScheduleAssociationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snapshotscheduleassociation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnapshotScheduleAssociation")
...
func (r *SnapshotScheduleAssociationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redshiftv1alpha1.SnapshotScheduleAssociation{}); err != nil {
			klog.Error(err, "unable to set up auditor", redshiftv1alpha1.SnapshotScheduleAssociation{}.APIVersion, redshiftv1alpha1.SnapshotScheduleAssociation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redshiftv1alpha1.SnapshotScheduleAssociation{}).
