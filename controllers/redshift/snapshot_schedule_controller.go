...
// +kubebuilder:rbac:groups=redshift.aws.kubeform.com,resources=snapshotschedules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=redshift.aws.kubeform.com,resources=snapshotschedules/status,verbs=get;update;patch

func (r *SnapshotScheduleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("snapshotschedule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SnapshotSchedule")
...
func (r *SnapshotScheduleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &redshiftv1alpha1.SnapshotSchedule{}); err != nil {
			klog.Error(err, "unable to set up auditor", redshiftv1alpha1.SnapshotSchedule{}.APIVersion, redshiftv1alpha1.SnapshotSchedule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&redshiftv1alpha1.SnapshotSchedule{}).
