...
// ElbReconciler reconciles a Elb object
type ElbReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=elb.aws.kubeform.com,resources=elbs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=elb.aws.kubeform.com,resources=elbs/status,verbs=get;update;patch

func (r *ElbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("elb", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Elb")
...
func (r *ElbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &elbv1alpha1.Elb{}); err != nil {
			klog.Error(err, "unable to set up auditor", elbv1alpha1.Elb{}.APIVersion, elbv1alpha1.Elb{}.Kind)
			return err
		}
	}
