...
// HostedConfigurationVersionReconciler reconciles a HostedConfigurationVersion object
type HostedConfigurationVersionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=hostedconfigurationversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=hostedconfigurationversions/status,verbs=get;update;patch

func (r *HostedConfigurationVersionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostedconfigurationversion", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostedConfigurationVersion")
...
func (r *HostedConfigurationVersionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appconfigv1alpha1.HostedConfigurationVersion{}); err != nil {
			klog.Error(err, "unable to set up auditor", appconfigv1alpha1.HostedConfigurationVersion{}.APIVersion, appconfigv1alpha1.HostedConfigurationVersion{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appconfigv1alpha1.HostedConfigurationVersion{}).
...
func (r *HostedConfigurationVersionReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for HostedConfigurationVersion")
			return nil
		}
		...
		resourceList := &appconfigv1alpha1.HostedConfigurationVersionList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list HostedConfigurationVersion")
			return nil
		}
