...
// ConfigurationProfileReconciler reconciles a ConfigurationProfile object
type ConfigurationProfileReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=configurationprofiles,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=configurationprofiles/status,verbs=get;update;patch

func (r *ConfigurationProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configurationprofile", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigurationProfile")
...
func (r *ConfigurationProfileReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appconfigv1alpha1.ConfigurationProfile{}); err != nil {
			klog.Error(err, "unable to set up auditor", appconfigv1alpha1.ConfigurationProfile{}.APIVersion, appconfigv1alpha1.ConfigurationProfile{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appconfigv1alpha1.ConfigurationProfile{}).
