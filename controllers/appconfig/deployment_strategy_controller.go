...
// DeploymentStrategyReconciler reconciles a DeploymentStrategy object
type DeploymentStrategyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=deploymentstrategies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=appconfig.aws.kubeform.com,resources=deploymentstrategies/status,verbs=get;update;patch

func (r *DeploymentStrategyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("deploymentstrategy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DeploymentStrategy")
...
func (r *DeploymentStrategyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appconfigv1alpha1.DeploymentStrategy{}); err != nil {
			klog.Error(err, "unable to set up auditor", appconfigv1alpha1.DeploymentStrategy{}.APIVersion, appconfigv1alpha1.DeploymentStrategy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appconfigv1alpha1.DeploymentStrategy{}).
