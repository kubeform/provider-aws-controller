...
// CookieStickinessPolicyReconciler reconciles a CookieStickinessPolicy object
type CookieStickinessPolicyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=app.aws.kubeform.com,resources=cookiestickinesspolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=app.aws.kubeform.com,resources=cookiestickinesspolicies/status,verbs=get;update;patch

func (r *CookieStickinessPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cookiestickinesspolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CookieStickinessPolicy")
...
func (r *CookieStickinessPolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &appv1alpha1.CookieStickinessPolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", appv1alpha1.CookieStickinessPolicy{}.APIVersion, appv1alpha1.CookieStickinessPolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&appv1alpha1.CookieStickinessPolicy{}).
