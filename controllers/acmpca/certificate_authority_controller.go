...
// +kubebuilder:rbac:groups=acmpca.aws.kubeform.com,resources=certificateauthorities,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=acmpca.aws.kubeform.com,resources=certificateauthorities/status,verbs=get;update;patch

func (r *CertificateAuthorityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificateauthority", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateAuthority")
...
func (r *CertificateAuthorityReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &acmpcav1alpha1.CertificateAuthority{}); err != nil {
			klog.Error(err, "unable to set up auditor", acmpcav1alpha1.CertificateAuthority{}.APIVersion, acmpcav1alpha1.CertificateAuthority{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&acmpcav1alpha1.CertificateAuthority{}).
