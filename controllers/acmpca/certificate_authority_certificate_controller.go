...
// CertificateAuthorityCertificateReconciler reconciles a CertificateAuthorityCertificate object
type CertificateAuthorityCertificateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=acmpca.aws.kubeform.com,resources=certificateauthoritycertificates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=acmpca.aws.kubeform.com,resources=certificateauthoritycertificates/status,verbs=get;update;patch

func (r *CertificateAuthorityCertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("certificateauthoritycertificate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CertificateAuthorityCertificate")
...
func (r *CertificateAuthorityCertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &acmpcav1alpha1.CertificateAuthorityCertificate{}); err != nil {
			klog.Error(err, "unable to set up auditor", acmpcav1alpha1.CertificateAuthorityCertificate{}.APIVersion, acmpcav1alpha1.CertificateAuthorityCertificate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&acmpcav1alpha1.CertificateAuthorityCertificate{}).
