...
// +kubebuilder:rbac:groups=budgets.aws.kubeform.com,resources=budgets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=budgets.aws.kubeform.com,resources=budgets/status,verbs=get;update;patch

func (r *BudgetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("budget", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Budget")
...
func (r *BudgetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &budgetsv1alpha1.Budget{}); err != nil {
			klog.Error(err, "unable to set up auditor", budgetsv1alpha1.Budget{}.APIVersion, budgetsv1alpha1.Budget{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&budgetsv1alpha1.Budget{}).
