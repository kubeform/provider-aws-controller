...
// BudgetActionReconciler reconciles a BudgetAction object
type BudgetActionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=budgets.aws.kubeform.com,resources=budgetactions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=budgets.aws.kubeform.com,resources=budgetactions/status,verbs=get;update;patch

func (r *BudgetActionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("budgetaction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BudgetAction")
...
func (r *BudgetActionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &budgetsv1alpha1.BudgetAction{}); err != nil {
			klog.Error(err, "unable to set up auditor", budgetsv1alpha1.BudgetAction{}.APIVersion, budgetsv1alpha1.BudgetAction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&budgetsv1alpha1.BudgetAction{}).
