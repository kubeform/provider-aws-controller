...
// EnvironmentMembershipReconciler reconciles a EnvironmentMembership object
type EnvironmentMembershipReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud9.aws.kubeform.com,resources=environmentmemberships,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud9.aws.kubeform.com,resources=environmentmemberships/status,verbs=get;update;patch

func (r *EnvironmentMembershipReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("environmentmembership", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EnvironmentMembership")
...
func (r *EnvironmentMembershipReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloud9v1alpha1.EnvironmentMembership{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloud9v1alpha1.EnvironmentMembership{}.APIVersion, cloud9v1alpha1.EnvironmentMembership{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloud9v1alpha1.EnvironmentMembership{}).
