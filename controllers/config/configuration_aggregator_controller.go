...
// ConfigurationAggregatorReconciler reconciles a ConfigurationAggregator object
type ConfigurationAggregatorReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=configurationaggregators,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=configurationaggregators/status,verbs=get;update;patch

func (r *ConfigurationAggregatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configurationaggregator", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigurationAggregator")
...
func (r *ConfigurationAggregatorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.ConfigurationAggregator{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.ConfigurationAggregator{}.APIVersion, configv1alpha1.ConfigurationAggregator{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.ConfigurationAggregator{}).
