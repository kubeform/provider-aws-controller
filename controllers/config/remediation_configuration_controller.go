...
// RemediationConfigurationReconciler reconciles a RemediationConfiguration object
type RemediationConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=remediationconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=remediationconfigurations/status,verbs=get;update;patch

func (r *RemediationConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("remediationconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RemediationConfiguration")
...
func (r *RemediationConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.RemediationConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.RemediationConfiguration{}.APIVersion, configv1alpha1.RemediationConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.RemediationConfiguration{}).
