...
// ConfigRuleReconciler reconciles a ConfigRule object
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=configrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=configrules/status,verbs=get;update;patch

func (r *ConfigRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("configrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConfigRule")
...
func (r *ConfigRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.ConfigRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.ConfigRule{}.APIVersion, configv1alpha1.ConfigRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.ConfigRule{}).
