...
// OrganizationConformancePackReconciler reconciles a OrganizationConformancePack object
type OrganizationConformancePackReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=organizationconformancepacks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=organizationconformancepacks/status,verbs=get;update;patch

func (r *OrganizationConformancePackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationconformancepack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationConformancePack")
...
func (r *OrganizationConformancePackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.OrganizationConformancePack{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.OrganizationConformancePack{}.APIVersion, configv1alpha1.OrganizationConformancePack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.OrganizationConformancePack{}).
