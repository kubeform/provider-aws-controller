...
// OrganizationCustomRuleReconciler reconciles a OrganizationCustomRule object
type OrganizationCustomRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=organizationcustomrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=organizationcustomrules/status,verbs=get;update;patch

func (r *OrganizationCustomRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("organizationcustomrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch OrganizationCustomRule")
...
func (r *OrganizationCustomRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.OrganizationCustomRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.OrganizationCustomRule{}.APIVersion, configv1alpha1.OrganizationCustomRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.OrganizationCustomRule{}).
