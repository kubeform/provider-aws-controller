...
// AggregateAuthorizationReconciler reconciles a AggregateAuthorization object
type AggregateAuthorizationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=aggregateauthorizations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=config.aws.kubeform.com,resources=aggregateauthorizations/status,verbs=get;update;patch

func (r *AggregateAuthorizationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("aggregateauthorization", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AggregateAuthorization")
...
func (r *AggregateAuthorizationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &configv1alpha1.AggregateAuthorization{}); err != nil {
			klog.Error(err, "unable to set up auditor", configv1alpha1.AggregateAuthorization{}.APIVersion, configv1alpha1.AggregateAuthorization{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&configv1alpha1.AggregateAuthorization{}).
