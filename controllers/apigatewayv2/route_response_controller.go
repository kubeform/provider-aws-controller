...
// RouteResponseReconciler reconciles a RouteResponse object
type RouteResponseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=apigatewayv2.aws.kubeform.com,resources=routeresponses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apigatewayv2.aws.kubeform.com,resources=routeresponses/status,verbs=get;update;patch

func (r *RouteResponseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("routeresponse", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch RouteResponse")
...
func (r *RouteResponseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apigatewayv2v1alpha1.RouteResponse{}); err != nil {
			klog.Error(err, "unable to set up auditor", apigatewayv2v1alpha1.RouteResponse{}.APIVersion, apigatewayv2v1alpha1.RouteResponse{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&apigatewayv2v1alpha1.RouteResponse{}).
