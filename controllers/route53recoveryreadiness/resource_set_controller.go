...
// +kubebuilder:rbac:groups=route53recoveryreadiness.aws.kubeform.com,resources=resourcesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53recoveryreadiness.aws.kubeform.com,resources=resourcesets/status,verbs=get;update;patch

func (r *ResourceSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("resourceset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ResourceSet")
...
func (r *ResourceSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53recoveryreadinessv1alpha1.ResourceSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53recoveryreadinessv1alpha1.ResourceSet{}.APIVersion, route53recoveryreadinessv1alpha1.ResourceSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53recoveryreadinessv1alpha1.ResourceSet{}).
