...
// ReadinessCheckReconciler reconciles a ReadinessCheck object
...
// +kubebuilder:rbac:groups=route53recoveryreadiness.aws.kubeform.com,resources=readinesschecks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=route53recoveryreadiness.aws.kubeform.com,resources=readinesschecks/status,verbs=get;update;patch

func (r *ReadinessCheckReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("readinesscheck", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReadinessCheck")
...
func (r *ReadinessCheckReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &route53recoveryreadinessv1alpha1.ReadinessCheck{}); err != nil {
			klog.Error(err, "unable to set up auditor", route53recoveryreadinessv1alpha1.ReadinessCheck{}.APIVersion, route53recoveryreadinessv1alpha1.ReadinessCheck{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&route53recoveryreadinessv1alpha1.ReadinessCheck{}).
