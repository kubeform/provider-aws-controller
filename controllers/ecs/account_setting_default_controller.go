...
// AccountSettingDefaultReconciler reconciles a AccountSettingDefault object
type AccountSettingDefaultReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=accountsettingdefaults,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=accountsettingdefaults/status,verbs=get;update;patch

func (r *AccountSettingDefaultReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("accountsettingdefault", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AccountSettingDefault")
...
func (r *AccountSettingDefaultReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.AccountSettingDefault{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.AccountSettingDefault{}.APIVersion, ecsv1alpha1.AccountSettingDefault{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.AccountSettingDefault{}).
