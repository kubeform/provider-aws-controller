...
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=taskdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=taskdefinitions/status,verbs=get;update;patch

func (r *TaskDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("taskdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TaskDefinition")
...
func (r *TaskDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.TaskDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.TaskDefinition{}.APIVersion, ecsv1alpha1.TaskDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.TaskDefinition{}).
