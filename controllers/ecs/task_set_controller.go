...
// TaskSetReconciler reconciles a TaskSet object
type TaskSetReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=tasksets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=tasksets/status,verbs=get;update;patch

func (r *TaskSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("taskset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TaskSet")
...
func (r *TaskSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.TaskSet{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.TaskSet{}.APIVersion, ecsv1alpha1.TaskSet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.TaskSet{}).
