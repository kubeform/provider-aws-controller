...
// ClusterCapacityProvidersReconciler reconciles a ClusterCapacityProviders object
type ClusterCapacityProvidersReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=clustercapacityproviders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ecs.aws.kubeform.com,resources=clustercapacityproviders/status,verbs=get;update;patch

func (r *ClusterCapacityProvidersReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustercapacityproviders", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterCapacityProviders")
...
func (r *ClusterCapacityProvidersReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ecsv1alpha1.ClusterCapacityProviders{}); err != nil {
			klog.Error(err, "unable to set up auditor", ecsv1alpha1.ClusterCapacityProviders{}.APIVersion, ecsv1alpha1.ClusterCapacityProviders{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ecsv1alpha1.ClusterCapacityProviders{}).
