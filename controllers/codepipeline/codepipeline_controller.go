...
// CodepipelineReconciler reconciles a Codepipeline object
...
// +kubebuilder:rbac:groups=codepipeline.aws.kubeform.com,resources=codepipelines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=codepipeline.aws.kubeform.com,resources=codepipelines/status,verbs=get;update;patch

func (r *CodepipelineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("codepipeline", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Codepipeline")
...
func (r *CodepipelineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &codepipelinev1alpha1.Codepipeline{}); err != nil {
			klog.Error(err, "unable to set up auditor", codepipelinev1alpha1.Codepipeline{}.APIVersion, codepipelinev1alpha1.Codepipeline{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&codepipelinev1alpha1.Codepipeline{}).
