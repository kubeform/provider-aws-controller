...
// +kubebuilder:rbac:groups=cur.aws.kubeform.com,resources=reportdefinitions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cur.aws.kubeform.com,resources=reportdefinitions/status,verbs=get;update;patch

func (r *ReportDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reportdefinition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReportDefinition")
...
func (r *ReportDefinitionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &curv1alpha1.ReportDefinition{}); err != nil {
			klog.Error(err, "unable to set up auditor", curv1alpha1.ReportDefinition{}.APIVersion, curv1alpha1.ReportDefinition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&curv1alpha1.ReportDefinition{}).
