...
// GlobalClusterReconciler reconciles a GlobalCluster object
type GlobalClusterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=docdb.aws.kubeform.com,resources=globalclusters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=docdb.aws.kubeform.com,resources=globalclusters/status,verbs=get;update;patch

func (r *GlobalClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globalcluster", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalCluster")
...
func (r *GlobalClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &docdbv1alpha1.GlobalCluster{}); err != nil {
			klog.Error(err, "unable to set up auditor", docdbv1alpha1.GlobalCluster{}.APIVersion, docdbv1alpha1.GlobalCluster{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&docdbv1alpha1.GlobalCluster{}).
