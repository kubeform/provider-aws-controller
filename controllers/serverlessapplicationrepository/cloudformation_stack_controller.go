...
// +kubebuilder:rbac:groups=serverlessapplicationrepository.aws.kubeform.com,resources=cloudformationstacks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=serverlessapplicationrepository.aws.kubeform.com,resources=cloudformationstacks/status,verbs=get;update;patch

func (r *CloudformationStackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudformationstack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CloudformationStack")
...
func (r *CloudformationStackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &serverlessapplicationrepositoryv1alpha1.CloudformationStack{}); err != nil {
			klog.Error(err, "unable to set up auditor", serverlessapplicationrepositoryv1alpha1.CloudformationStack{}.APIVersion, serverlessapplicationrepositoryv1alpha1.CloudformationStack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&serverlessapplicationrepositoryv1alpha1.CloudformationStack{}).
